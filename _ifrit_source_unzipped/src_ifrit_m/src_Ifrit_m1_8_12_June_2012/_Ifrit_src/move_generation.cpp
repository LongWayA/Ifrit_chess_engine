...
//	===================================================================
//
void Move_generation::ini_attacks_magic_masks_database()
{
...
//	===================================================================
//
unsigned __int64 Move_generation::ini_deg_masks
(
...
unsigned __int64 Move_generation::ini_rook_attacks_masks
(
	const int square, 
...
unsigned __int64 Move_generation::ini_bishop_attacks_masks
(
	const int square, 
	const unsigned __int64 deg_masks
)
...
//	===================================================================
//
inline unsigned __int64 Move_generation::get_bishop_attacks_masks_magic
(
...
//	===================================================================
//
inline unsigned __int64 Move_generation::get_rook_attacks_masks_magic
(
	const unsigned __int8 & square,/// расположение фигуры 
	const unsigned __int64 & position_all_piece ///
)
...
void Move_generation::generator_captures_white
(
...
void Move_generation::generator_captures_black
(
	struct List & list,// представление списка ходов
	const struct Position & position // представление доски   
)
...
//(для белых)
void Move_generation::generator_move_white
(
	struct List & list,// представление списка ходов
	const struct Position & position // представление доски   
)
...
void Move_generation::generator_move_black
(
	struct List & list,// представление списка ходов
	const struct Position & position // представление доски   
)
...
unsigned __int8 Move_generation::king_white_check
(
	const struct Position & position // представление доски   
)
{
...
//	===================================================================
// под боем ли король если шаха нет то функция возвращает 0 иначе цифру шахующей фигуры
unsigned __int8 Move_generation::king_black_check
(
	const struct Position & position // представление доски   
)
{
...
inline unsigned __int8 Move_generation::white_check_square
(
...
inline unsigned __int8 Move_generation::black_check_square
(
	const unsigned __int8 & sq,//индекс проверяемого поля(0-63) 
	const struct Position & position // представление доски   
)
...
inline void Move_generation::list_capture_white
(
	struct List & list,// представление списка ходов
...
)
{
	...
	//-------------------------------------------------------------------------------
	//std::cout << "move[count]= "<< move[count] << std::endl;
	//
	//std::cout << "position.black_name_from_coord[to]= "<< position.black_name_from_coord[to] << std::endl;
	//std::cout << "position.black_name_from_coord[to]= "<< ( move[p] & 63) << std::endl;
	//
	//std::cout << "type_move= "<< type_move << std::endl;
	//std::cout << "type_move= "<< ((move[count]>>6)& 63)  << std::endl;
	//
	//std::cout << "from= "<< from << std::endl;
	//std::cout << "from= "<< ((move[count]>>12)& 63)  << std::endl;
	//
	//std::cout << "to= "<< to << std::endl;
	//std::cout << "to= "<< ((move[count]>>18)& 63)  << std::endl;
	//
	//std::cout << "name= "<< name << std::endl;
	//std::cout << "name= "<<  ((move[count]>>24)& 7)  << std::endl;
	//-------------------------------------------------------------------------------
...
// две функции только из за разного контроля списков(строки взятия)
inline void Move_generation::list_capture_black
(
	struct List & list,// представление списка ходов
	const unsigned __int8 name,// имя ходящей фигуры
	const unsigned __int8 type_move,// тип хода
...
)
{
	...
	//std::cout << "move[count]= "<< move[count] << std::endl;
	//
	//std::cout << "position.white_name_from_coord[to]= "<< position.white_name_from_coord[to] << std::endl;
	//std::cout << "position.white_name_from_coord[to]= "<< ( move[p] & 63) << std::endl;
	//
	//std::cout << "type_move= "<< type_move << std::endl;
	//std::cout << "type_move= "<< ((move[count]>>6)& 63)  << std::endl;
	//
...
//	===================================================================
// заполняем список ходов
inline void Move_generation::list_record_move_white
(
	struct List & list,// представление списка ходов
	const unsigned __int8 name,// имя ходящей фигуры
	const unsigned __int8 type_move,// тип хода
...
)
{
...
#if SW_HISTORY
	//  записываем историю
	list.sorting_score[count] = History_heuristic::get_white_history(name,to); 
#endif//#if SW_HISTORY
	...
	//-------------------------------------------------------------------------------
	//std::cout << "move[p]= "<< move[p] << std::endl;
	//
	//std::cout << "0= "<< 0 << std::endl;
	//std::cout << "0= "<< ( move[p] & 63) << std::endl;
	//
	//std::cout << "d_m= "<< d_m << std::endl;
	//std::cout << "d_m= "<< ((move[p]>>6)& 63)  << std::endl;
	//
	//std::cout << "from= "<< from << std::endl;
	//std::cout << "from= "<< ((move[p]>>12)& 63)  << std::endl;
	//
	//std::cout << "to= "<< to << std::endl;
	//std::cout << "to= "<< ((move[p]>>18)& 63)  << std::endl;
	//
	//std::cout << "name= "<< name << std::endl;
	//std::cout << "name= "<<  ((move[p]>>24)& 7)  << std::endl;
...
//	===================================================================
// заполняем список ходов
inline void Move_generation::list_record_move_black
(
	struct List & list,// представление списка ходов
	const unsigned __int8 name,// имя ходящей фигуры
	const unsigned __int8 type_move,// тип хода
...
#if SW_HISTORY
	// записываем историю
	list.sorting_score[count] =  History_heuristic::get_black_history(name,to); 
#endif//#if SW_HISTORY
...
inline void Move_generation::list_record_special
(
	struct List & list,// представление списка ходов
	const unsigned __int8 name,// имя ходящей фигуры
...
inline void Move_generation::KONTROL_captures_W
(
	const struct Position & position, // представление доски   
...
)
{
	...
	if ((p < 0)||(p > 256))
	{
		std::cout << "W ERROR Generator_captures p= "   << p    << std::endl;
	}

	if ((from < 0)||(from > 63))
	{
		std::cout << "W ERROR Generator_captures from= " << from << std::endl;
	}

	if ((to < 0)||(to > 63))
	{
		std::cout << "W ERROR Generator_captures to= "  << to   << std::endl;
	}
	...
	||(position.white_name_from_coord[from] > 6))
	{
		std::cout << "W ERROR Generator_captures position.white_name_from_coord[*from] = "  
			<< position.white_name_from_coord[from]<< std::endl;
	}
	...
	||(position.black_name_from_coord[to] > 6))
	{
		std::cout << "W ERROR Generator_captures position.black_name_from_coord[*to]= " 
			<< position.black_name_from_coord[to]   << std::endl;
		std::cout << "W ERROR Generator_captures from= "<< from << std::endl;
		std::cout << "W ERROR Generator_captures to= "  << to   << std::endl;
		std::cout << "W ERROR Generator_captures name_figure= "  << name_figure << std::endl;
		position_print(position);
	}

	//if (position.black_name_from_coord[to] ==6)
	//{
		//std::cout << "ERROR Generator_captures name_figure KING "  << std::endl;
	//}
	///////////////////////////////////////////////////////////////////////////////////
#endif//KONTROL_B
}
...
inline void Move_generation::KONTROL_captures_B
(
	const struct Position & position, // представление доски   
	const unsigned __int8 & from,// откуда ходит фигура
	const unsigned __int8 & to,// куда ходит фигура
...
)
{
	...
	if ((p < 0)||(p > 256))
	{
		std::cout << "B ERROR Generator_captures p= "   << p    << std::endl;
	}

	if ((from < 0)||(from > 63))
	{
		std::cout << "B ERROR Generator_captures from= " << from << std::endl;
	}

	if ((to < 0)||(to > 63))
	{
		std::cout << "B ERROR Generator_captures to= "  << to   << std::endl;
	}
	...
	||(position.black_name_from_coord[from] > 6))
	{
		std::cout << "B ERROR Generator_captures position.black_name_from_coord[from] = "
			<< position.black_name_from_coord[from]   << std::endl;
	}
	...
	||(position.white_name_from_coord[to] > 6))
	{
		std::cout << "B ERROR Generator_captures position.white_name_from_coord[to]= "
			<< position.white_name_from_coord[to]   << std::endl;
		std::cout << "B ERROR Generator_captures from= "<< from << std::endl;
		std::cout << "B ERROR Generator_captures to= "  << to   << std::endl;
		std::cout << "B ERROR Generator_captures name_figure= "  << name_figure << std::endl;
		position_print(position);
	}

	//if (position.white_name_from_coord[to] ==6)
	//{
	//std::cout << "B ERROR Generator_captures name_figure KING "  << std::endl;
	//}
///////////////////////////////////////////////////////////////////////////////////
#endif//KONTROL_B
}
...
inline void Move_generation::KONTROL_move
(
	const unsigned __int8 & from,// откуда ходит фигура
	const unsigned __int8 & to,// куда ходит фигура
	const unsigned __int8 & p// номер хода в списке
)
{
	...
	if ((p < 0)||(p > 256))
	{
		std::cout << "ERROR Generator_move p= "   << p    << std::endl;
	}

	if ((from < 0)||(from > 63))
	{
		std::cout << "ERROR Generator_move from= " << from << std::endl;
	}

	if ((to < 0)||(to > 63))
	{
		std::cout << "ERROR Generator_move to= "  << to   << std::endl;
	}
	///////////////////////////////////////////////////////////////////////////////////
#endif//KONTROL_B
}
...
__inline __int32 Move_generation::bit_scan_forward_win32
(
...
__inline __int32 Move_generation::bit_scan_reverse_win32
(
	const unsigned __int64 analyzed_number// число в котором ищем взведенный бит
)
{ 
