...
)
{
	...
	*/
	...
	if (from_mask != 0)
	{
		from = Move_generation::bit_scan_forward_win32(from_mask);
		from_white_king = from;
		//cout << "короли " << std::endl;
		//cout << "from= "<< from << std::endl;
		//cout << "position_k_e[from]= "<< position_k_e[from] << std::endl;
		...
		//std::cout << "from= "<< (int)from << std::endl;
		...
		&& ((from == G1) || (from == F1)) ) //  белый король в зоне рокировки 0-0
		{
			...
			//std::cout << "oo from= "<< (int)from << std::endl;
		...
		&& ((from == B1) || (from == C1) || (from == D1))) //  белый король в зоне рокировки 0-0-0
		{
			...
			//std::cout << "ooo from= "<< (int)from << std::endl;
			//std::cout << "eval= "<< eval << std::endl;
		}
		//-----------------------------------------------------------

		eval = eval 
			+ position_king_white_opening[from] 
	...
	if (from_mask != 0)
	{
		...
		eval = eval 
			- position_king_black_opening[from] 
...
)
{
	...
	*/
	...
	if (from_mask != 0)
	{
		...
		eval = eval + position_king_white_endgame[from];
	}
	...
	if (from_mask != 0)
	{
		...
		// после рубикона король должен стремиться к центру
		eval = eval - position_king_black_endgame[from];
	}
}
...
)
{
	...
	while (from_mask != 0)
	{
		...
		{// если сделали короткую рокировку
			eval = eval + position_pawn_king_in_oo_white[from];
		}
		else if ((from_white_king == A1) || (from_white_king == B1) 
		|| (from_white_king == C1))
		{// если сделали длинную рокировку
			eval = eval + position_pawn_king_in_ooo_white[from];
		}
		else
		{// если рокировки не получилось
			eval = eval + position_pawn_king_in_center_white[from];
		}
		...
		{ 
			...
			+ position_perspective_pawn_white_end[from]; 
		}
		...
		//std::cout << "w from % 8= "<< from % 8 << std::endl;
	}
...
// черные-------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{// если сделали короткую рокировку
			eval = eval - position_pawn_king_in_oo_black[from];
		}
		else if ((from_black_king == A8) || (from_black_king == B8) 
		|| (from_black_king == C8))
		{// если сделали длинную рокировку
			eval = eval - position_pawn_king_in_ooo_black[from];
		}
		else
		{// если рокировки не получилось
			eval = eval - position_pawn_king_in_center_black[from];
		}
		...
		{ 
			...
			if (cnt_line_pawn_black == 2) 
				eval = eval - position_perspective_pawn_black_end[from];
		}

		// если есть сдвоенные пешки то в массиве будут двойки если строенные то тройки за них штраф
		Structure_pawn_b[from % 8] = Structure_pawn_b[from % 8] + 1;
		//std::cout << "b from % 8= "<< from % 8 << std::endl;
	}
	...
	for (int ii = 0; ii < 8; ii++)
	{
		...
		//std::cout << "Structure_pawn_w[ii]= "<< Structure_pawn_w[ii] << std::endl;
		//std::cout << "Structure_pawn_b[ii]= "<< Structure_pawn_b[ii] << std::endl;
	}
...
)
{
	...
	*/
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_pawn_white_end[from];
		//eval = eval + test[from];
		//cout << "from= "<< from << std::endl;
		//cout << "test[from]= "<< test[from] << std::endl;
		//cout << "from % 8= "<< from % 8 << std::endl;
...
// черные-------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_pawn_black_end[from];
...
)
{
	...
	*/
	...
	while (from_mask != 0)
	{
		...
		unsigned __int64 move_mask = Move_generation::get_rook_attacks_masks_magic(from,position.all_piece);
		unsigned __int64 move_mask_b = Move_generation::get_bishop_attacks_masks_magic(from,position.all_piece);
		move_mask = move_mask | move_mask_b;
		move_mask = move_mask & (~position.all_white_piece);

		eval = eval + position_queen_white_opening[from]
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_rook_white_opening[from] 
	...
	while (from_mask != 0)
	{
		...
		unsigned __int64 move_mask = Move_generation::get_bishop_attacks_masks_magic(from,position.all_piece);
		move_mask = move_mask & (~position.all_white_piece);

		eval = eval + position_bishop_white_opening[from] 
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_knigh_white_opening[from] 
...
/////////////////////////////////////
	...
	while (from_mask != 0)
	{
		...
		eval = eval  - position_queen_black_opening[from] 
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_rook_black_opening[from]   
		- CONTROL_SQUARE_ROOK_OPENING * popcnt(move_mask)
		;
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_bishop_black_opening[from] 
		- CONTROL_SQUARE_BISHOP_OPENING * popcnt(move_mask)
		;
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_knigh_black_opening[from] 
		- CONTROL_SQUARE_KNIGHT_OPENING * popcnt(move_mask)
		;
...
)
{
	...
	*/
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_queen_white_endgame[from]
	...
	while (from_mask != 0)
	{
		...
		eval = eval //+ position_rook_white_endgame[from] 
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_bishop_white_endgame[from] 
	...
	while (from_mask != 0)
	{
		...
		eval = eval + position_knigh_white_endgame[from] 
...
/////////////////////////////////////
	...
	while (from_mask != 0)
	{
		...
		eval = eval  - position_queen_black_endgame[from] 
	...
	while (from_mask != 0)
	{
		...
		eval = eval //- position_rook_black_endgame[from]   
		- CONTROL_SQUARE_ROOK_ENDGAME * popcnt(move_mask)
		;
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_bishop_black_endgame[from] 
		- CONTROL_SQUARE_BISHOP_ENDGAME * popcnt(move_mask)
		;
	...
	while (from_mask != 0)
	{
		...
		eval = eval - position_knigh_black_endgame[from] 
		- CONTROL_SQUARE_KNIGHT_ENDGAME * popcnt(move_mask)
		;
...
)
{
	...
	//{
	//	cout << "KARAUUUUUUUUUUUUUUL"<< std::endl;
	//}
	//--------------------------------------------------------------------------
...
{ 
	...
	//	//std::cout << "popcnt= " << popcnt(e_move_masks[i]) << std::endl;
	//	std::cout << "popcnt= " << popcnt(move) << std::endl;
	//}
    // TEST-------------------------------------
...
//std::cout <<std::endl;
//std::cout << " from = " << (int)from <<std::endl;
//std::cout << " knigt_moves_masks " <<std::endl;
//Test_chess_bitboard::print_bitboard_map(move_mask);
//std::cout <<std::endl;
//std::cout << " black_king_move_mask " <<std::endl;
