...
/*
 разбираем фен - позицию и инициализируем движок используем 
 подфункцию ch_koordinata_to_indeks
...
*/
void Uci_fen_parser::fen_to_board
(
...
)
{
	...
	// TEST
	//std::cout << "--------------------" << std::endl;
	//std::cout << position_fen.at(i-1) << std::endl;
	//std::cout << position_fen.at(i) << std::endl;
	//std::cout << position_fen.at(i+1) << std::endl;
	//std::cout << "--------------------" << std::endl;
		...
		while ((position_fen.at(i) != '/') && (position_fen.at(i) != ' '))
		{
			...
			switch (position_fen.at(i))
			{
				...
				default:
					...
					// если зашло '8' то функция ch_koordinata_to_indeks 
					// выдает цифру 7 на же нужна цифра 8 
					// поэтому используем (1 +... 
					void_q = (1 + ch_koordinata_to_indeks(position_fen.at(i)));

					// обработали первую пустую клетку
					// и следующие 7 ( таким образом у нас обработаны 
					// 1 + 7 = 8 все пустые клетки ) 
	...
	// сразу присваиваем запрет на рокировку который 
	// отменяем ниже если есть нужные символы типа К
	...
	rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1
	другими словами нам просто передают 
	координаты битого поля (при ходе е4 битое поле е3)
	*/
	// проматываем пробел
	while(position_fen.at(i) == ' ')
	{
	...
	if (position_fen.at(i) != '-')
	{
		...
		position.taking_passage_x = 
			koordinata_to_indeks(position_fen.at(i));// х координата(из буквы)
		i = i + 1;
		position.taking_passage_y = 
			ch_koordinata_to_indeks(position_fen.at(i));// у координата 	
	}
	//---------------------------------------------------------------------------
...
*/
int Uci_fen_parser::koordinata_to_indeks
(
	const __int8 ch
 )
{
...
*/
int Uci_fen_parser::ch_koordinata_to_indeks
(
 const int ch
 )
{
...
*/
void Uci_fen_parser::bitboard_ini
(
	struct Position & position// представление доски
)
{
