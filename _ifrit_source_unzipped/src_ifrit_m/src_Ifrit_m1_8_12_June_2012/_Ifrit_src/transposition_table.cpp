...
// ПОСЛЕДНЕЕ ИЗМЕНЕНИЕ 9.03.2009 20.1.2010 3.6.2012
...
void HASHM::test_hash_print()
{
	...
	std::cout << "hash_write_all = " << hash_write_all << std::endl;

	if (hash_write_all != 0)
	{
		// количество записанных позиций для сортировки ходов
		std::cout << "hash_move_write = " << hash_move_write << "   100 * hash_move_write/hash_write_all = " << 100 * hash_move_write/hash_write_all << std::endl;

		// количество записанных позиций для отсечки по оценке
		std::cout << "hash_score_write_low = " << hash_score_write_low << "   100 * hash_score_write_low/hash_write_all = " << 100 * hash_score_write_low/hash_write_all << std::endl;

		// количество записанных позиций для отсечки по оценке
		std::cout << "hash_score_write_high = " << hash_score_write_high << "   100 * hash_score_write_high/hash_write_all = " << 100 * hash_score_write_high/hash_write_all << std::endl;

		std::cout << " " << std::endl;

		// печатаем количество использованных для сортировки позиций
		std::cout << "hash_read_sorting = " << hash_read_sorting << "   100 * hash_read_sorting/hash_write_all = " << 100 * hash_read_sorting/hash_write_all << std::endl;
	}

	// дельта между записанными и использованными
	std::cout << "hash_write_all - hash_read_sorting = " << (hash_write_all - hash_read_sorting) << std::endl;
	...
	if ( hash_read_score != 0)
	{
		if (hash_write_all != 0)
		{
			...
			std::cout << "hash_read_score = " << hash_read_score << "   100 * hash_read_score/hash_write_all = " << 100 * hash_read_score/hash_write_all << std::endl;

			// печатаем количество всех считанных позиций
			std::cout << "hash_read_all = " << (hash_read_sorting + hash_read_score) << "   100 * hash_read_all/hash_write_all = " << 100 * (hash_read_sorting + hash_read_score)/hash_write_all << std::endl;
		}

		// печатаем количество использованных для отсчеки по оценки позиций
		std::cout << "hash_read_score_LOWER = " << hash_read_score_LOWER <<  "   100 * hash_read_score_LOWER/hash_read_score = " << 100 * hash_read_score_LOWER/hash_read_score << std::endl;

		// печатаем количество использованных для отсчеки по оценки позиций
		std::cout << "hash_read_score_UPPER = " << hash_read_score_UPPER <<  "   100 * hash_read_score_UPPER/hash_read_score = " << 100 * hash_read_score_UPPER/hash_read_score << std::endl;
	}
...
bool Transposition_table::get_flag_found_score()
{
	return flag_found_score;
}


//	===================================================================
// выделяем память под кэш. размер задается в мегабайтах
void Transposition_table::hash_size_ini_default
(
...
)
{
	...
	// функция sizeof(structHash_moves) выдает размер в байтах т.е. int это 4
	// в данный момент structHash_moves - 16 байт
	// перегоняем размер в Мб в байты а потом в количество элементов
	max_index_hash = (unsigned __int64)(
		(hash_size * (1024 * 1024)) / sizeof(struct Hash_moves));

	//std::cout << "max_index_hash in 1 mb = " << max_index_hash << std::endl;
	//std::cout << "sizeof(Hash_moves) = " << sizeof(struct Hash_moves) << std::endl;
	...
	{
		std::cout << "ЗАСАДА! размер не вида степени двух " << hash_n << std::endl;
	}
	...
	p_hash_moves = (struct Hash_moves*) calloc (
		(__int32)(max_index_hash + 3), sizeof(struct Hash_moves));

	if (p_hash_moves == NULL) 
	{
		std::cout << "памяти нет " << std::endl;
		exit(1); 
	}

	// TEST
	//std::cout << "sizeof(Hash_moves) = " << sizeof(Hash_moves) << std::endl;
	//std::cout << "max_index_hash = " << max_index_hash << std::endl;
	//std::cout << "hash_n = " << hash_n << std::endl;
...
void Transposition_table::hash_size_resize
(
	unsigned __int16 hash_size// размер хеш-таблицы в мегабайтах
)
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "освобождать нечего " << std::endl;
		exit(1); 
	}
    ...
    // TEST
	//std::cout << "предварительный max_index_hash = " << max_index_hash << std::endl;
...
// TEST
	//std::cout << " размер хеш-таблицы в мегабайтах hash_size = " << hash_size << std::endl;
	//std::cout << " в итоге выделили памяти в мб = " << ((int)(max_index_hash + 3) * sizeof(Hash_moves))/(1024 * 1024) << std::endl;
	//std::cout << "размер элемента в байтах sizeof(Hash_moves) = " << sizeof(Hash_moves) << std::endl;
	//std::cout << "число степени 2 hash_n = " << hash_n << std::endl;
	//std::cout << "количество элементов в хеш-таблице max_index_hash = " << max_index_hash << std::endl;

}

//	===================================================================
// освобождаем память из под хеш-таблицы
void Transposition_table::hash_size_free()
{
	if (p_hash_moves != NULL)
	{
		free(p_hash_moves);
...
void Transposition_table::ini_hash_moves()
{    
	if (p_hash_moves != NULL)
	{
...
void Transposition_table::save_best_hash_score_or_move
(
...
)
{
	...
	*/
	...
	// запись лучшего хода
	save_best_hash_move(i,key_lock_32,list_surplus_moves,delta_depth,ind);
...
//	===================================================================
// кэшируем лучший на данный момент ход или оценку (зависит от поиска)
void Transposition_table::save_best_hash_move
(
	unsigned __int8 i,// номер хода по списку
	unsigned __int64 key,// хеш-ключ
	const struct List & list_surplus_moves,//список возможных ходов
...
/*
 смотрим не встречалась ли нам такая позиция если встречалась то применяем 
 сохраненную оценку
 поиск за белых
*/
__int32 Transposition_table::look_hash_score_white
(
	...
	const __int32 alpha,//
	const __int32 beta,//
	unsigned __int8 & test_flag_hash_moves,// флаг хода (1- улучшили альфу(бету); 
										   // 3-просто локальный максимум(минимум); 
...
)
{
	...
	2)EXACT (проверяем score  > alpha )&&(проверяем score < beta)
	...
	максимум(в случае белых).
	*/

	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::look_hash_score_white  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	if (p_hash_moves[ind].key_lock_32_score == key_lock_32)
	{
		...
		/*
			если позиция уже рассмотренна на длину варианта больше 
			или равную той на которую
			мы собираемся просмотреть то можно просто 
			вернуть оценку варианта
		*/
		if (p_hash_moves[ind].score_delta_depth >= (depth_max - depth))
		{
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == BETA_CUTOFF)
				)
			{

				/*
					LOWER (проверяем score >= beta )
					была отсечка по бете
					нам надо проверить что условие осечки все еще соблюдается	
				*/
			...
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == ALPHA_UPDATED)
				)
			{
				/*
					EXACT (проверяем score  > alpha )
					&&(p_hash_moves[ind].score < beta)
					понятно что отсечки по бете не было
					а альфа улучшилась значит мы имеем точную оценку
					но мы имеем нулевое окно так что это точная оценка 
					на самом деле не точна
					однако это самая достоверная оценка потому что мы
					ее в любом случае будем пересчитывать
				*/
			...
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == LOCAL_MAX)
				)
			{
				/*
					UPPER (проверяем score < alpha )
					наверх эта оценка не пойдет
				*/
				if (p_hash_moves[ind].score < alpha)
				{ 
#if TEST_WRITE_READ
...
//поиск за черных
__int32 Transposition_table::look_hash_score_black
(
	const unsigned __int64 key,// хеш-ключ
	const unsigned __int8 depth,// глубина
	const unsigned __int8 depth_max,// максимальная глубина
...
)
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::look_hash_score_black  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	if (p_hash_moves[ind].key_lock_32_score == key_lock_32)
	{
		...
		{
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == ALPHA_CUTOFF)
				)
			{
				if (p_hash_moves[ind].score <= alpha) 
				{ 
			...
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == BETA_UPDATED)
				)
			{
				if ((p_hash_moves[ind].score < beta) 
					&&(p_hash_moves[ind].score > alpha)) 
				{ 
#if TEST_WRITE_READ
					// 
					hash_read_score_LOWER = hash_read_score_LOWER + 1;
			...
			//--------------------------------------------------------
			if ((p_hash_moves[ind].flag_hash_score == LOCAL_MIN)
				)
			{
				if (p_hash_moves[ind].score > beta) 
				{ 
...
//	===================================================================
// смотрим не встречалась ли нам такая позиция если встречалась то ее ход смотрим первым
void Transposition_table::look_hash_move
(
...
)
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::look_hash_move  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	if (p_hash_moves[ind].key_lock_32_move == (unsigned __int32)(key >> 32))
	{
		...
		insert_hash_moves(list_surplus_moves,p_hash_moves[ind].move,flag_insert_hash_move);
	}
}

//	===================================================================
// обнуляем возраст позиции
void Transposition_table::set_age_0()
{
	age = 0;
}
...
void Transposition_table::aging()
{
	if (age > 250) age = 0;

	age = age + 1;
	//std::cout << "age= " << (__int32)age << std::endl;
}
...
inline void Transposition_table::save_best_hash_move
(
	unsigned __int8 i,// номер хода по списку
...
)
{
	...
	*/
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::save_best_hash_move  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	if (p_hash_moves[ind].key_lock_32_move == 0)
	{
		...
		save_hash_move(i,key_lock_32,list_surplus_moves,delta_depth,ind);
...
//	===================================================================
// кэшируем лучший на данный момент ход 
inline void Transposition_table::save_hash_move
(
	unsigned __int8 i,// номер хода по списку
	unsigned __int32 key_lock_32,//
	const struct List & list_surplus_moves,//список возможных ходов
...
)
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::save_hash_move  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	p_hash_moves[ind].move = list_surplus_moves.move[i];
...
inline void Transposition_table::save_best_hash_score
(
	unsigned __int32 key_lock_32,//
	__int32 score,// оценка хода
	unsigned __int8 flag_hash_score,// флаг хода (1- улучшили альфу(бету); 3-просто локальный максимум(минимум); 2-отсечка по бете(альфе))
...
)
{
	...
	*/
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::save_best_hash_score  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
...
inline void Transposition_table::save_hash_score
(
	unsigned __int32 key_lock_32,//
	__int32 score,// оценка хода
	unsigned __int8 flag_hash_score,// флаг хода (1- улучшили альфу(бету); 3-просто локальный максимум(минимум); 2-отсечка по бете(альфе))
...
)
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::save_hash_score  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
...
void Transposition_table::TEST_Hash_index_print()
{
	...
	if (p_hash_moves == NULL) 
	{
		std::cout << "Transposition_table::TEST_Hash_index_print  p_hash_moves == NULL " << std::endl;
		exit(1); 
	}
	...
	std::cout << "  "<< std::endl;
	std::cout << " полное количество элементов " << max_index_hash << std::endl;

	if (full_move != 0)
	{
		std::cout << "full_move = " << full_move << std::endl;
	}

	if (full_score != 0)
	{
		std::cout << "full_score = " << full_score << std::endl;
	}
	...
	if (yes_move != 0)
	{
		std::cout << "yes_move = " << yes_move << std::endl;
	}

	if (yes_score != 0) 
	{
		std::cout << "yes_score = " << yes_score << std::endl;
	}
	...
	if (no_move != 0)
	{
		std::cout << "no_move = " << no_move << std::endl;
	}

	if (no_score != 0)
	{
		std::cout << "no_score = " << no_score << std::endl;
	}
	...
	if (no_move != 0)
	{
		std::cout << " 100 * yes_move/no_move = " << 100 * yes_move/no_move << std::endl;
	}

	if (no_score != 0)
	{
		std::cout << " 100 * yes_score/no_score = " << 100 * yes_score/no_score << std::endl;
	}
...
void Transposition_table::insert_hash_moves
(
...
//TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
		...
		{
			if(hash_moves[ihm].key_lock_32_move != -1) pozition = pozition + 1;
		}

		Test_Hash << "Всего позиций "<< max_index_hash <<std::endl;
		Test_Hash << "Для записей использовали позиций "<< pozition <<std::endl;

		move_print = hash_moves[ind].move;

		kolizii = kolizii + 1;

		Test_Hash << "Вхождения "<< enter <<std::endl;
		Test_Hash << "Коллизия "<< kolizii <<std::endl;
		Test_Hash << "depth "<< depth <<std::endl;
		Test_Hash << "hash_moves[].move_delta_depth " << hash_moves[ind].move_delta_depth << std::endl;
		//Test_Hash << "key "<< key <<std::endl;
		Test_Hash << "hash_moves[].key " << hash_moves[ind].key_lock_32_move << std::endl;
		Test_Hash << "hash_moves[].move "<< move_print <<std::endl;

		//------------------------------------------
		Test_Hash << "Ходящая фигура " << UTIL::public_int_to_char_board(((move_print>>24)& 7),1)<< std::endl;
		Test_Hash << "hash_moves[].score_move " << hash_moves[ind].score << std::endl;

		// вывод хода вида e2-e4
		//------------------------------
		Test_Hash << " " << (UTIL::public_x_to_notation(((move_print>>12)& 63)%8));
		Test_Hash << "" << (1 + ((move_print>>12)& 63)/8);//<< std::endl;
		//Test_Hash<< std::endl;
		Test_Hash << " " << (UTIL::public_x_to_notation(((move_print>>18)& 63)%8));
		Test_Hash << "" << (1 + ((move_print>>18)& 63)/8)<< std::endl;   
		//------------------------------
		//тип хода 
		...
		Test_Hash << "вид хода " << ((move_print>>6)& 63)  << std::endl;
		Test_Hash << "Взятая фигура " << UTIL::public_int_to_char_board(( move_print & 63),1)<< std::endl;
		//Test_Hash << "начальная положение " << ((move_print>>12)& 63)  << std::endl;
		//Test_Hash << "конечное положение " << ((move_print>>18)& 63)  << std::endl;
     
		Test_Hash << std::endl;
		//------------------------------------------

		Test_Hash << "end_list= "<< end_list <<std::endl;
		Test_Hash << "j= "<< j <<std::endl;
		Test_Hash << "-----------------------------------"<<std::endl;
