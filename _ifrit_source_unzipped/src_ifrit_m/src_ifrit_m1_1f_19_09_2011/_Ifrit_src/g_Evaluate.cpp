...
// ПОСЛЕДНЕЕ ИЗМЕНЕНИЕ 24.02.2009 03.07.2010 19.09.2011
...
)
{
	...
	*/
	...
	unsigned __int64 white_king_move_mask = king_moves_masks[from_white_king] & (~position.all_piece);

	// находим маску ходов короля(без взятий)
	unsigned __int64 black_king_move_mask = king_moves_masks[from_black_king] & (~position.all_piece);
	...
	while (from_mask != 0)
	{
		...
		+ CONTROL_SQUARE_QUEEN * popcnt(move_mask) // max r14 + b13 = q27  n8 k8
		//+ test[from] 
		;
	...
	while (from_mask != 0)
	{
		...
		+ CONTROL_SQUARE_ROOK * popcnt(move_mask)
		//+ test[from] 
		;
	...
	while (from_mask != 0)
	{
		...
		+ CONTROL_SQUARE_BISHOP * popcnt(move_mask)
		;
	...
	while (from_mask != 0)
	{
		...
		unsigned __int64 move_mask = knigt_moves_masks[from] & (~position.all_piece); // маска мобильности

		eval = eval + pozicion_knigh_white[from] 
		+ CONTROL_SQUARE_KNIGHT * popcnt(move_mask)
		;
...
/////////////////////////////////////
	...
	while (from_mask != 0)
	{
		...
		- CONTROL_SQUARE_QUEEN * popcnt(move_mask)
		;
