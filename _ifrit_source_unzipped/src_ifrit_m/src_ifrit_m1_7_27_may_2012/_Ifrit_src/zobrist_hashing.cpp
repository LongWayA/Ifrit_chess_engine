	...
	for (unsigned __int8 color_piece = 0; color_piece < 2; color_piece++)
	{
		for (unsigned __int8 name_piece = 1; name_piece < 7; name_piece++)
		{
			...
			coordinate_piece < 64; coordinate_piece++)
			{
				...
				//Test_Hash << random_piese_key[color_piece][name_piece][coordinate_piece] << std::endl;
			}
		}
	}
...
)
{
	...
	//		std::cout << "white_name name = " << name << " i = " << i << "  random_piese_key[1][name][i] = " << random_piese_key[1][name][i] << std::endl;
	//	}
	...
	//	//	std::cout << "black_name name = " << name << std::endl;
	//	//}
	//}
	...
	for (unsigned __int8 i = 0; i < 64; i++)
	{
		...
		{
			key = key ^ random_piese_key[0][name][i];
			//std::cout << "black_name name = " << name << " i = " << i << "  random_piese_key[1][name][i] = " << random_piese_key[1][name][i] << std::endl;
		}
	}
	...
	//std::cout << "bitboard.hash_key = " << bitboard.hash_key << std::endl;
	//std::cout << "   "<< std::endl;
}
...
//  , 205,    227, 229, 230, 232, 234
	...
	//  )
	//{
	//	std::cout << "Ifrit: Zobrist_hashing::modification_random_key bug. I am terminated." << std::endl;
	//	exit(1);
	//}
	...
	//<< "undo.return_castling_K = " << undo.return_castling_K<< std::endl;
	//if (bitboard.castling_Q != undo.return_castling_Q) std::cout << "bitboard.castling_Q = " << bitboard.castling_Q 
	//<< "undo.return_castling_Q = " << undo.return_castling_Q<< std::endl;
	//if (bitboard.castling_k != undo.return_castling_k)  std::cout << "bitboard.castling_k = " << bitboard.castling_k 
	//<< "undo.return_castling_k = " << undo.return_castling_k<< std::endl;
	//if (bitboard.castling_q != undo.return_castling_q)  std::cout << "bitboard.castling_q = " << bitboard.castling_q 
	//<< "undo.return_castling_q = " << undo.return_castling_q<< std::endl;
