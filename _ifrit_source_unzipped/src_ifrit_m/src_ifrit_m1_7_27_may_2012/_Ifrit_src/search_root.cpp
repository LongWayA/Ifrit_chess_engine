...
//_scout_scout white
		...
		else
		{
			...
			&& ( depth_max_new >= LMR_DEPTH ))// достаточная глубина
			{
				...
				//
				value = Search::black(position,alpha,beta_scout,
									depth_max_new_scout,new_depth
									,description_move,
									flag_pv_f_scout,flag_check,NULL_NO);
				//
...
//_scout_scout black
...
#endif//#if ALPHA_BETA_ON
		{
			...
			value = Search::white(position,alpha_scout,beta,
								depth_max_new_scout,new_depth,description_move,
								flag_pv_f_scout,flag_check,NULL_NO);
		}
		else
		{
