...
)
{
	...
	//{
	//	std::cout << "WHITE eval_white_material != bitboard.value_white_material" <<std::endl;
	//	std::cout << "eval_white_material(position)= "<< Evaluate::eval_white_material(position)<<std::endl;
	//	std::cout << "position.value_white_material= "<< position.value_white_material<<std::endl;
	//	exit(1);
	//}

	//if (Evaluate::eval_black_material(position) != position.value_black_material)
	//{
	//	std::cout << "WHITE eval_black_material != position.value_black_material" <<std::endl;
	//	std::cout << "eval_black_material(position)= "<< Evaluate::eval_black_material(position)<<std::endl;
	//	std::cout << "position.value_black_material= "<< position.value_black_material<<std::endl;
	//	exit(1);
	//}
	...
	//{
	//	std::cout << "WHITE eval_difference_material != (position.value_white_material - position.value_black_material)" <<std::endl;
	//	std::cout << "eval_difference_material(position)= "<< Evaluate::eval_difference_material(position)<<std::endl;
	//	std::cout << "(position.value_white_material - position.value_black_material)= " << (position.value_white_material - position.value_black_material)<<std::endl;
	//	exit(1);
	//}
	//------------------------------------------------
	...
	*/
	...
	{
		//TEST
		//cout << "white depth= " << depth<<std::endl;
		//cout << "i = " << i <<std::endl;
		//cout << "list_surplus_moves.move[i] & 63 = " << (list_surplus_moves.move[i] & 63) <<std::endl;
...
)
{
	...
	{
		//TEST
		//cout << "black depth= " << depth<<std::endl;
		//cout << "i = " << i <<std::endl;
		//cout << "list_surplus_moves.move[i] & 63 = " << (list_surplus_moves.move[i] & 63) <<std::endl;
...
)
{
	...
	Test_Loop_f << std::endl;
	Test_Loop_f << "------------------------------------------------- "<<std::endl;
	Test_Loop_f <<"depth= "<< depth << std::endl;
	Test_Loop_f << std::endl;
	Test_Loop_f << "Список возможных ходов фигуры "<<std::endl;

	for (int i = 0 ; i < list_surplus_moves->end_list ; i++)
	{	
		Test_Loop_f << "Номер хода по списку i= " << i << std::endl;  
		Test_Loop_f << "Ходящая фигура " << UTIL::public_int_to_char_board(((list_surplus_moves->move[i]>>24)& 7),1)<< std::endl;
		Test_Loop_f << "Оценка хода " << list_surplus_moves->sorting_score[i] << std::endl;

		// вывод хода вида e2-e4
		//------------------------------
		Test_Loop_f << " " << (UTIL::public_x_to_notation(((list_surplus_moves->move[i]>>12)& 63)%8));
		Test_Loop_f << "" << (1 + ((list_surplus_moves->move[i]>>12)& 63)/8);//<< std::endl;
		//Test_Loop_f << std::endl;
		Test_Loop_f << " " << (UTIL::public_x_to_notation(((list_surplus_moves->move[i]>>18)& 63)%8));
		Test_Loop_f << "" << (1 + ((list_surplus_moves->move[i]>>18)& 63)/8)<< std::endl;   
		//------------------------------

		Test_Loop_f << "вид хода " << ((list_surplus_moves->move[i]>>6)& 63)  << std::endl;
		Test_Loop_f << "Взятая фигура " << UTIL::public_int_to_char_board(( list_surplus_moves->move[i] & 63),1)<< std::endl;
		Test_Loop_f << "начальная положение " << ((list_surplus_moves->move[i]>>12)& 63)  << std::endl;
		Test_Loop_f << "конечное положение " << ((list_surplus_moves->move[i]>>18)& 63)  << std::endl;
      
		Test_Loop_f << std::endl;
	}

	Test_Loop_f << "list_surplus_moves->end_captures= " <<list_surplus_moves->end_captures << std::endl;
	Test_Loop_f << "list_surplus_moves->end_list= " <<list_surplus_moves->end_list << std::endl;
	Test_Loop_f << "------------------------------------------------- " << std::endl;
	//GGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGGG
	Test_Loop_f.close();
}
