...
void Test_chess_bitboard::print_bitboard_map
(
...
void Test_chess_bitboard::print_name_from_coord
(
...
void Test_chess_bitboard::save_test_bitboard
(
...
void Test_chess_bitboard::compare_test_bitboard
(
...
)
{
	...
	{
		std::cout << "white test_all_piece != bitboard->all_piece" << std::endl;// все фигуры
	}

	if (position_1.all_white_piece != position_2.all_white_piece)
	{
		std::cout << "white test_all_white_piece != bitboard->all_white_piece" << std::endl;// все белые фигуры
	}

	if (position_1.all_black_piece != position_2.all_black_piece) 
	{
		std::cout << "white test_all_black_piece != bitboard->all_black_piece" << std::endl;// все черные фигуры
	}

	if (position_1.colour_move != position_2.colour_move)
	{
		std::cout << "white test_colour_move != bitboard->colour_move" << std::endl;
	}

	if (position_1.taking_passage_yes != position_2.taking_passage_yes)
	{
		std::cout << "white test_taking_passage_yes != bitboard->taking_passage_yes" << std::endl;
	}

	if (position_1.taking_passage_x != position_2.taking_passage_x)
	{
		std::cout << "white test_taking_passage_x != bitboard->taking_passage_x" << std::endl;
	}

	if (position_1.taking_passage_y != position_2.taking_passage_y)
	{
		std::cout << "white test_taking_passage_y != bitboard->taking_passage_y" << std::endl;
	}

	if (position_1.castling_Q != position_2.castling_Q)
	{
		std::cout << "white test_castling_Q != bitboard->castling_Q" << std::endl;
	}

	if (position_1.castling_K != position_2.castling_K)
	{
		std::cout << "white test_castling_K != bitboard->castling_K" << std::endl;
	}
	...
	{
		std::cout << "white test_hash_key !=  bitboard->hash_key" << std::endl;
	}

	if (position_1.value_white_material != position_2.value_white_material)
	{
		std::cout << "white test_value_white_material != bitboard->value_white_material" << std::endl;
	}

	if (position_1.value_black_material != position_2.value_black_material)
	{
		std::cout << "white test_value_black_material != bitboard->value_black_material" << std::endl;
	}
	...
	for (unsigned __int8 n2 = 0; n2 < 7; n2++ )
	{
		...
		{
			std::cout << "white test_white_piece[n] = bitboard->white_piece[n]" << n2 << std::endl;
		}

		if (position_1.black_piece[n2] != position_2.black_piece[n2])
		{
			std::cout << "white test_black_piece[n] = bitboard->black_piece[n]" << n2 << std::endl;
		}
	}
	...
	for (unsigned __int8 k2 = 0; k2 < 64; k2++ )
	{
		...
		{
			std::cout << "white test_white_name_from_coord[k] = bitboard->white_name_from_coord[k]" << k2 << std::endl;
		}

		if (position_1.black_name_from_coord[k2] != position_2.black_name_from_coord[k2])
		{
			std::cout << "white test_black_name_from_coord[k] = bitboard->black_name_from_coord[k]" << k2 << std::endl;
		}
	}
...
void Test_chess_bitboard::bitboard_print
(
	const struct Position & position// представление доски
)
{
	...
	Test_Bitboard << "bitboard->colour_move = "<< position.colour_move <<std::endl;
	Test_Bitboard << std::endl;

	Test_Bitboard << "taking_passage_yes= " << position.taking_passage_yes << std::endl;
	Test_Bitboard << "bitboard->taking_passage_x= " << (int)position.taking_passage_x << std::endl;
	Test_Bitboard << "taking_passage_y= " << (int)position.taking_passage_y << std::endl;

	Test_Bitboard << std::endl;
	Test_Bitboard << "castling_K= " << position.castling_K << std::endl;
	Test_Bitboard << "castling_Q= " << position.castling_Q << std::endl;
	Test_Bitboard << "castling_k= " << position.castling_k << std::endl;
	Test_Bitboard << "castling_q= " << position.castling_q << std::endl;
	...
	//-------------------------------------------------------------------------------------
	Test_Bitboard << std::endl;
	Test_Bitboard << "all_white_piece"<< std::endl;
	...
	//-------------------------------------------------------------------------------------
	Test_Bitboard << std::endl;
	Test_Bitboard << "all_black_piece"<< std::endl;
	...
	//-------------------------------------------------------------------------------------
	Test_Bitboard << std::endl;
	Test_Bitboard << "white_piece"<< std::endl;
	for (int p = 1; p < 7; p++)
	{
		Test_Bitboard << std::endl;
		...
		if (p == 1)
		{
			Test_Bitboard << "PAWN" << std::endl;
		}
		else if (p == 2)
		{
			Test_Bitboard << "KNIGHT" << std::endl;
		}
		else if (p == 3)
		{
			Test_Bitboard << "BISHOP" << std::endl;
		}
		else if (p == 4)
		{
			Test_Bitboard << "ROOK" << std::endl;
		}
		else if (p == 5)
		{
			Test_Bitboard << "QUEEN" << std::endl;
		}
		else if (p == 6)
		{
			Test_Bitboard << "KING" << std::endl;
		}
	...
	//-------------------------------------------------------------------------------------
	Test_Bitboard << std::endl;
	Test_Bitboard << "black_piece"<< std::endl;
