...
//----------------------------
		...
		extension_for_chek_white(position,depth_max,depth_max_new,flag_check,old_flag_check);
...
//_scout_scout white
		...
		else
		{
			...
			if ((description_move == MOVE_IS_SIMPLE)// простой ход
			&& (i > FULL_SEARCH)//
			&& (flag_check == CHECK_NO) 
			//&& (old_flag_check == CHECK_NO)//
			&& ((depth_max_new - depth) >= LMR_DEPTH ))//
...
//----------------------------
		...
		extension_for_chek_black(position,depth_max,depth_max_new,flag_check,old_flag_check);
#endif//#if EXTENSION_CHEK
...
)
{
	...
	else
	{
		...
		//flag_check = (flag_check || old_flag_check);
}
