...
void Uci_parser::parse_protocol_UCI
(
...
//TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
	...
	else if ( (int)string_in.compare(0,8,"position") == 0 )
	{
		...
		Fen_parser::bitboard_ini(position);
...
//  TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
		...
		Uci_go::parse_go(position,string_in);
...
unsigned __int16 Uci_parser::read_hash_size_Mb
(
	const std::string string_in//входящая строка
)
{
...
void Uci_parser::parse_position_FEN
(
	struct Position & position,// представление доски
	const std::string string_in//входящая строка
)
{
	...
	if ( (int)string_in.compare(0,12,"position fen") == 0 )
	{
		...
		Fen_parser::fen_to_board(position,string_in);
	...
	else
	{
		...
		Fen_parser::fen_to_board(position,start_fen);
	}
...
)
{
	...
	unsigned __int8 n = 8 * Fen_parser::ch_koordinata_to_indeks(move_string[2]) 
		+ Fen_parser::koordinata_to_indeks(move_string[1]);// откуда фигура ходит

	unsigned __int8 n_2 = 8 * Fen_parser::ch_koordinata_to_indeks(move_string[4]) 
		+ Fen_parser::koordinata_to_indeks(move_string[3]);// куда фигура ходит
	...
	for (unsigned __int8 i = 0; i < list_surplus_moves.end_list; i++)
	{
		...
		if (( n == ( (list_surplus_moves.move[i] >> 12) & 63 ) ) && // совпадение откуда фигура ходит
		( n_2 == ((list_surplus_moves.move[i] >> 18) & 63) ) )  // совпадение куда фигура ходит
		{
			/*
