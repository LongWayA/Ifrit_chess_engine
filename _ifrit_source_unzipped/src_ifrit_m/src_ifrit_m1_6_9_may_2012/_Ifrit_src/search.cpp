...
// 7 генерируем и сортируем список избыточных ходов
	...
	//{
	//	Killer_heuristic::insert_killer(list_surplus_moves,depth,0);
	//}
	//else
	//{
	//	Killer_heuristic::insert_killer(list_surplus_moves,depth,1);
	//}
...
//_scout_scout white
   ...
   //         (i > 5)
			...
			//&& (list_surplus_moves.move[i] != Killer_heuristic::get_killer_moves_1(depth))// не киллер
			//&& (list_surplus_moves.move[i] != Killer_heuristic::get_killer_moves_2(depth))
...
#endif//#if KILLER
#if SW_HISTORY
						History_heuristic::white_history_good_save(i,list_surplus_moves,depth,depth_max);
#endif//#if SW_HISTORY
					}
...
//#endif//#if LMR
	...
	for (unsigned __int8 i = 0; i < list_surplus_moves.end_list; i++)
	{
		...
		Zobrist_hashing::modification_random_key(position,0,i,list_surplus_moves,Make_move::get_d_undo(depth));
...
//_scout_scout black
   ...
   //         (i > 5)
			...
			//&& (piece_move != N_KING)  // ход не королем
			&& (flag_check == CHECK_NO)
			&& (list_surplus_moves.move[i] != Killer_heuristic::get_killer_moves_1(depth))
			&& (list_surplus_moves.move[i] != Killer_heuristic::get_killer_moves_2(depth))
			&& ((depth_max_new - depth) >= LMR_DEPTH ))
			{
				alpha_scout = beta - 1;//
...
#endif//#if KILLER
#if SW_HISTORY
						History_heuristic::black_history_good_save(i,list_surplus_moves,depth,depth_max);
#endif//#if SW_HISTORY
					}
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	...
	if ( flag_save_best == true ) 
	{
		...
		Transposition_table::save_best_hash_score_or_move(i_hash_moves_1,position.hash_key,list_surplus_moves, 
			beta, EXACT, depth, depth_max);
	}
	else
	{
		// кэшируем ход дающий локальный минимум т.е. флаг 3
		Transposition_table::save_best_hash_score_or_move(i_hash_moves_3,position.hash_key,list_surplus_moves, 
