...
// ПОСЛЕДНЕЕ ИЗМЕНЕНИЕ 21.02.2009  21.11.2010 18.09.2011
...
class i_Test_chess_bitboard test_gen;//


//	===================================================================
//
void h_Generator_moves::ini_attacks_magic_masks_database()
{
...
//	===================================================================
//
unsigned __int64 h_Generator_moves::ini_deg_masks
(
...
unsigned __int64 h_Generator_moves::ini_rook_attacks_masks
(
	const int square, 
...
unsigned __int64 h_Generator_moves::ini_bishop_attacks_masks
(
	const int square, 
	const unsigned __int64 deg_masks
)
...
//	===================================================================
//
inline unsigned __int64 h_Generator_moves::get_bishop_attacks_masks_magic
(
...
//	===================================================================
//
inline unsigned __int64 h_Generator_moves::get_rook_attacks_masks_magic
(
	const unsigned __int8 & square,/// расположение фигуры 
	const unsigned __int64 & position_all_piece ///
)
...
)
{
	...
	if (from_mask != 0)
	{
		...
		//move_mask = position.white_mobility_from_coord[from];
		mobility_king(move_mask,position,from);
	...
	while (from_mask != 0)
	{
		...
		//move_mask = position.white_mobility_from_coord[from];
        mobility_queen(move_mask,position,from);
	...
	while (from_mask != 0)
	{
		...
		//move_mask = position.white_mobility_from_coord[from];
		mobility_rook(move_mask,position,from);
	...
	while (from_mask != 0)
	{
		...
		//move_mask = position.white_mobility_from_coord[from];
		mobility_bishop(move_mask,position,from);
	...
	while (from_mask != 0)
	{
		...
		//move_mask = position.white_mobility_from_coord[from];
        mobility_knigt(move_mask,position,from);
...
)
{
	...
	if (from_mask != 0)
	{
		...
		//move_mask = position.black_mobility_from_coord[from];
		mobility_king(move_mask,position,from);
...
//	===================================================================
// 
__inline void h_Generator_moves::mobility_king
(
	unsigned __int64 & mobility_mask, // маска мобильности
	const struct Position & position, /// представление доски    
	const unsigned __int8 & square/// расположение фигуры     
)
{
		// находим маску ходов короля(без взятий)
		mobility_mask = king_moves_masks[square] & (~position.all_piece);
...
//	===================================================================
// 
__inline void h_Generator_moves::mobility_queen
(
	unsigned __int64 & mobility_mask, // маска мобильности
	const struct Position & position, /// представление доски    
	const unsigned __int8 & square/// расположение фигуры     
)
{
	...
	mobility_mask = 0; // маска мобильности

	move_mask = get_rook_attacks_masks_magic(square,position.all_piece);
	move_mask = move_mask ^ (position.all_piece & move_mask);

	mobility_mask = get_bishop_attacks_masks_magic(square,position.all_piece);
	mobility_mask = mobility_mask ^ (position.all_piece & mobility_mask);

	mobility_mask = mobility_mask | move_mask;
}


//	===================================================================
// 
__inline void h_Generator_moves::mobility_rook
(
	unsigned __int64 & mobility_mask, // маска мобильности
	const struct Position & position, /// представление доски    
	const unsigned __int8 & square/// расположение фигуры     
)
{
	...
	mobility_mask = get_rook_attacks_masks_magic(square,position.all_piece);
	mobility_mask = mobility_mask ^ (position.all_piece & mobility_mask);
...
//	===================================================================
// 
__inline void h_Generator_moves::mobility_bishop
(
	unsigned __int64 & mobility_mask, // маска мобильности
	const struct Position & position, /// представление доски    
	const unsigned __int8 & square/// расположение фигуры     
...
//	===================================================================
// 
__inline void h_Generator_moves::mobility_knigt
(
	unsigned __int64 & mobility_mask, // маска мобильности
	const struct Position & position, /// представление доски    
	const unsigned __int8 & square/// расположение фигуры     
...
{
	mobility_mask = knigt_moves_masks[square] & (~position.all_piece); // маска мобильности
}


//	===================================================================
// 
void h_Generator_moves::mobility_evaluation
(
...
)
{
	...
	for (unsigned __int8 i = 0; i < MAX_COORDINATE; i++)
	{

		name_figure = position.white_name_from_coord[i];

		if (name_figure == N_KING)
		{
			mobility_king(mask_mobility_from_coord[i],position,i);
		}
		else if (name_figure == N_QUEEN)
		{
			mobility_queen(mask_mobility_from_coord[i],position,i);
		}
		else if (name_figure == N_ROOK)
		{
			mobility_rook(mask_mobility_from_coord[i],position,i);
		}
		else if (name_figure == N_BISHOP)
		{
			mobility_bishop(mask_mobility_from_coord[i],position,i);
		}
		else if (name_figure == N_KNIGHT)
		{
			mobility_knigt(mask_mobility_from_coord[i],position,i);
		}

//-------------------------------------

		name_figure = position.black_name_from_coord[i];
