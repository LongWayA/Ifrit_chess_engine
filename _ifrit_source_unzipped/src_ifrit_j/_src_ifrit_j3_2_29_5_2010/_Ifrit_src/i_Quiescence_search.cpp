...
)
{
	...
	{// взятие пешек исключаем
		...
		{// взятие слонов и коней исключаем
			...
			if (value_stat > beta + ROOK_MARGIN)// взятие ладьи исключаем
	...
	if (save_best == 1)
	{
		...
		else// теперь мы местный лучший вариант скопируем в структуру PV_best что бы передать на более высокий уровень
		{
			if(flag_pv_f) 
				PVSAVE::public_update_PV_best(&pv_best_point);
		}
