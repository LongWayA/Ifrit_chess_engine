...
//TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
	...
	p_bitboard->eval = EVALUATE::public_eval_forced(p_bitboard);
...
//---------------------------------------------------------------
		...
		// выходим по времени
		if ((time_terminate != -1) && (time_terminate < time_current) && (depth >= 2))
			break;
	...
	if (ROOTSEARCH::public_get_from() != ((ROOTSEARCH::public_get_pv_best()->move[0]>>12)& 63))
		std::cout << "ERROR from(Deepening)" <<"\n";

	if (ROOTSEARCH::public_get_to() != ((ROOTSEARCH::public_get_pv_best()->move[0]>>18)& 63))
