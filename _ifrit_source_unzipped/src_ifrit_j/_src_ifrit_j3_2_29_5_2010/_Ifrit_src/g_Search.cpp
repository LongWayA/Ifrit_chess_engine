...
namespace 
{
	...
	// FUTILITY PRUNING
	const unsigned __int8 FUTILITY_MARGIN = 120; // поле отсечки для futility pruning
	const unsigned __int8 FUTILITY_DEPTH = 1; // глубина срабатывания для futility pruning
...
//sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
	...
	if ((depth_max - depth) == FUTILITY_DEPTH)
	{
		...
		&& (flag_check == CHECK_NO) 
		&& (flag_pv_f == PV_NO))
			if ((p_bitboard->eval + EVALUATE::public_eval_state(p_bitboard)
			- FUTILITY_MARGIN) >= beta )
				return beta;
	...
	{// тогда вызываем нулевой ход
		...
		value = public_black(p_bitboard,alpha,(alpha + 1),(depth_max - NULL_REDUCTION),
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	...
	if (flag_save_best == 1)
		...
		if(flag_pv_f) PVSAVE::public_update_PV_best(&pv_best_point);
...
)
{
	...
	{
		// у нас получился вариант приводящий к данной позиции и плюс оценка этой позиции
		if (flag_pv_f) PVSAVE::public_update_PV_best_max_depth(0 // оценка ничья
															,depth);
		return 0;
	}
#endif//#if ALPHA_BETA_ON
	...
	if ((depth_max - depth) == FUTILITY_DEPTH)
	{
		...
		&& (flag_pv_f == PV_NO))
		{
			if ((p_bitboard->eval + EVALUATE::public_eval_state(p_bitboard) + FUTILITY_MARGIN) 
			<= alpha )
				return alpha;
		}
	...
	{ // тогда вызываем нулевой ход
		...
		value = public_white(p_bitboard,(beta - 1),beta,(depth_max - NULL_REDUCTION),
			new_depth,description_move, 
			PV_NO,old_flag_check,NULL_YES);
...
)
{
	...
	score = QUIESCENCESEARCH::public_forced_variant_white
		(p_bitboard,alpha,beta,depth,flag_pv_f,(depth + 5));
...
#if FORCED // включаем режим просчета только взятий
	//score = EVALUATE::public_eval_state(p_bitboard);
	//score_f = QUIESCENCESEARCH::Forced_variant_white
	//	(p_bitboard,(alpha - score),(beta + score),depth,(depth + 5));
...
#if !FORCED 
	score = EVALUATE::public_eval_state(p_bitboard) + p_bitboard->eval;
...
)
{
	...
	score = QUIESCENCESEARCH::public_forced_variant_black
		(p_bitboard,alpha,beta,depth,flag_pv_f,(depth + 5));
...
#if FORCED // включаем режим просчета только взятий
     //  score = EVALUATE::public_eval_state(p_bitboard);
  	  // score_f = QUIESCENCESEARCH::Forced_variant_black
		   //(p_bitboard,(alpha - score),(beta + score),depth,(depth + 5));
	...
	//у нас получился вариант приводящий к данной позиции и плюс оценка этой позиции
	//надо понимать что в pv_current ни оценки ни глубины не задается мы их сразу пишем в pv_best
	if (flag_pv_f) PVSAVE::public_update_PV_best_max_depth(score,depth);
