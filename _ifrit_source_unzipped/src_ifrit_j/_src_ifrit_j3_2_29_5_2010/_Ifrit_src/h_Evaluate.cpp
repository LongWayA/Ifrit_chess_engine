	...
	*/
	...
	if (from_mask != 0)
	{
		...
		if (stadia_b > rubikon)
		{
			...
			//if (((p_bitboard->white_piece[4] & 192) != 0) //  ладья заперта на 6 или 7 клетках
			...
			//if (((p_bitboard->white_piece[4] & 3) != 0) //  ладья заперта на 6 или 7 клетках
	...
	if (from_mask != 0)
	{
		...
		if (stadia_w > rubikon)
		{
			...
			//if (( (p_bitboard->black_piece[4] & 13835058055282163712) != 0 ) //  ладья заперта на 6 или 7 клетках
			...
			//if (( (p_bitboard->black_piece[4] & 216172782113783808) != 0 ) //  ладья заперта на 6 или 7 клетках
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (((from % 8) == 7) || ((from % 8) == 0)) 
				cnt_line_pawn_white = 1;

			// в случае если справо перед пешкой нет вражеской пешки
			if ((from % 8) != 7) 
				if (!(e_rook_up_masks[from + 1] & p_bitboard->black_piece[1]))
				{ 
			...
			if (cnt_line_pawn_white == 2) 
				eval = eval + end * Perspective_pawn_e[from]; 
...
// черные
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			// для случая крайней проходной
			if (((from % 8) == 7) || ((from % 8) == 0) ) 
				cnt_line_pawn_black = 1;

			// в случае если справо перед пешкой нет вражеской пешки
			if ((from % 8) != 7) 
				if (!(e_rook_down_masks[from + 1] & p_bitboard->white_piece[1]))
				{
					eval = eval - PERSPECTIVE_PAWNN;
			...
			if (cnt_line_pawn_black == 2) 
				eval = eval - end * Perspective_pawn_e[63 - from]; 
					//- 26 * PERSPECTIVE_PAWNN;
		}
	...
	{
		if (Structure_pawn_w[ii] > 1) 
			eval = eval - BONUS_PAWNN; // сейчас штраф 20

		if (Structure_pawn_b[ii] > 1) 
			eval = eval + BONUS_PAWNN;
...
)
{
	...
	if (z == 1)
	{
		...
		if (p_bitboard->white_name_from_coord[n + 7] == 0) 
			st = st + SHIELD;

		// смотрим на клетку влево вверх
		if (p_bitboard->white_name_from_coord[n + 8] == 0) 
			st = st + SHIELD;

		// смотрим на клетку вправо вверх
		if (p_bitboard->white_name_from_coord[n + 9] == 0) 
			st = st + SHIELD;
	}
	else
	{
		// смотрим на клетку вниз от короля
		if (p_bitboard->black_name_from_coord[n - 7] == 0) 
			st = st + SHIELD;

		// смотрим на клетку  влево вниз
		if (p_bitboard->black_name_from_coord[n - 8] == 0) 
			st = st + SHIELD;

		// смотрим на клетку вправо вниз
		if (p_bitboard->black_name_from_coord[n - 9] == 0) 
			st = st + SHIELD;
	}
...
__int32 EVALUATE::public_eval_forced
(
	const T_Bitboard * p_bitboard// представление доски
)
{
...
)
{
	...
	if ((e_king_moves_masks[from_king] & move_mask) != 0 ) 
		at_cnt = at_cnt + 1 ;
