...
//-----------------------------------------------
			...
			// здесь у нас классический негаскаут(NegaSstd::cout)
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
			...
			if (FULLSEARCH::public_get_stop_search() != 1)
			{
				...
				// для теста вариантов
				public_print_pv_line(1,&list_surplus_moves,PVSAVE::public_get_pv_best(),nodes,i,depth_max,value,time_nodes);
#else
				// для рабочего режима
				//PVSAVE::public_print_pv_line(1,&list_surplus_moves,&pv_best_point,nodes_root,i,depth_max,value_max);
...
#if TEST_PRINT_ALL_LINE
				// для теста вариантов
				public_print_pv_line(0,&list_surplus_moves,PVSAVE::public_get_pv_best(),nodes,i,depth_max,value,time_nodes);
#else
				// для рабочего режима
				//PVSAVE::public_print_pv_line(0,&list_surplus_moves,&pv_best_point,nodes_root,i,depth_max,value_min);
#endif//#if TEST_PRINT_ALL_LINE
			}
#endif//#if SAVE_LINE 
//sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
...
)
{
	...
	FULLSEARCH::public_ini_global_loop(depth_max);
}
...
)
{
	...
	if (p_test_bitboard->eval != p_bitboard->eval) 
		std::cout << "white test_eval != bitboard->eval" << "\n";
