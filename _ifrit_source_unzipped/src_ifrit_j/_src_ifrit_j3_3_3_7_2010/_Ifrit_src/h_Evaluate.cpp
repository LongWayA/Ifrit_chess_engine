	...
	*/
	...
	while (from_mask != 0)
	{
		...
		if (stadia_b > rubikon)
		{
			...
			else if ((from_white_king == A1) || (from_white_king == B1) || (from_white_king == C1))
		...
		{ 
			...
			if ((from % 8) != 7)
			{
				if (!(e_rook_up_masks[from + 1] & p_bitboard->black_piece[BITBOARDGENERATOR::N_PAWN]))
				{ 
			...
			if (cnt_line_pawn_white == 2) eval = eval + pozicion_perspective_pawn_white_end[from]; 
		}
...
// черные-------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		if (stadia_w > rubikon)
		{
			...
			else if ((from_black_king == A8) || (from_black_king == B8) || (from_black_king == C8))
			{// если сделали длинную рокировку
		...
		{ 
			...
			if ((from % 8) != 7)
			{
				if (!(e_rook_down_masks[from + 1] & p_bitboard->white_piece[BITBOARDGENERATOR::N_PAWN]))
				{
					eval = eval - PERSPECTIVE_PAWNN;
