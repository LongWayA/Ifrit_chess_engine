         ...
         // белые работают с альфой и пытаются ее улучшить
	         ...
	         if (value > alpha){ 
                ...
                if(value != pv_save_o.get_pv_best()->score_move){
                  ...
                  // кэшируем лучший на данный момент ход
                  hash_o.Save_best_hash(i, p_bitboard->hash_key, &list_surplus_moves, 
				                        value_max, EXACT, depth, depth_max);
                #endif//#if HASH
           ...
           //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
              ...
              if (value < beta){
                 ...
                 if (value != pv_save_o.get_pv_best()->score_move){
                   ...
                   //кэшируем лучший на данный момент ход
                   hash_o.Save_best_hash(i, p_bitboard->hash_key, &list_surplus_moves, 
					                     value_min, EXACT, depth, depth_max);
                 #endif//#if HASH
...
const unsigned __int8 new_depth = depth + 1; // следующая глубина
	...
	//sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
      ...
      unsigned __int8 hash_moves_1;// ход который улучшает альфу (1- улучшили альфу) нужно для кеширования 
      unsigned __int8 hash_moves_3;// ход дающий локальный максимум (3-просто локальный максимум )нужно для кеширования
...
//------------------------------
if ((depth_max - depth) < (VERIFICATOIN_SEARCH_REDUCTION + 1) ) return value;
...
//-----------------
         ...
         alpha = value;
         flag_save_best = 1;
         hash_moves_1 = i;// ход который улучшает альфу (1- улучшили альфу) нужно для кеширования 
    ...
    //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
       ...
       value_max = value;
        hash_moves_3 = i;// ход дающий локальный максимум (3-просто локальный максимум)нужно для кеширования
...
//--------------------------------------------
	...
	//sssssssssssssssssssssssssssssssssssssssssssssssssssssssssssss
	  ...
	  if( flag_save_best == 1 ){
	    ...
	    hash_o.Save_best_hash(hash_moves_1, p_bitboard->hash_key, &list_surplus_moves, 
			                  alpha, EXACT, depth, depth_max);
	  }else{
	    // кэшируем ход дающий локальный максимум т.е. флаг 3
	    hash_o.Save_best_hash(hash_moves_3, p_bitboard->hash_key, &list_surplus_moves, 
			                  value_max, UPPER, depth, depth_max);
	  }//if(flag_save_best == 1){
	#endif//#if HASH
...
const unsigned __int8 new_depth = depth + 1; // следующая глубина
    ...
    const unsigned __int64 key_undo = p_bitboard->hash_key;   // запоминаем хеш-ключ даннного узла
    __int32 hash_score = -1000000000;// хеш-оценка позиции
    unsigned __int8 hash_moves_1;// ход который улучшает бету (1- улучшили бету) нужно для кеширования 
    unsigned __int8 hash_moves_3;// ход дающий локальный минимум (3-просто локальный минимум)нужно для кеширования
    unsigned __int8 test_flag_hash_moves;// флаг хода (1- улучшили альфу(бету); 3-просто локальный максимум(минимум); 2-отсечка по бете(альфе))
    unsigned __int8 hash_depth;// максимальная глубина
    unsigned __int8 hash_depth_max;// максимальная глубина
...
//-------------------
           ...
           beta = value;
           flag_save_best = 1;
           hash_moves_1 = i;// ход который улучшает бету (1- улучшили бету) нужно для кеширования 
    ...
    //AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
       ...
       value_min = value;
       hash_moves_3 = i;// ход дающий локальный минимум (3-просто локальный минимум)нужно для кеширования
