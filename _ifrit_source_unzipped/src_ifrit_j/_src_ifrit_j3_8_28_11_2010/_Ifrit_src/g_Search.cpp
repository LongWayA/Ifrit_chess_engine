...
*/
	...
	{ 
		...
		&& (old_flag_check == CHECK_NO)) // если шаха нет
		{
			...
			//value = p_bitboard->value_white_material - p_bitboard->value_black_material
			//	+ futility_margin[(depth_max - depth)];

			//if (value < beta)
			//{
			//	value = QUIESCENCESEARCH::public_forced_variant_white(p_bitboard
	...
	list_surplus_moves.public_generator_captures_white(p_bitboard);
	list_surplus_moves.public_generator_move_white(p_bitboard);
	...
	{
		list_surplus_moves.public_insert_killer(depth,0);
	}
	else
	{
		list_surplus_moves.public_insert_killer(depth,1);
	}
#endif//#if KILLER
	...
	*/
	...
	{ 
		...
		&& (old_flag_check == CHECK_NO)) // если шаха нет
		{
			...
			//if (value > alpha)
			//{
			//	value = QUIESCENCESEARCH::public_forced_variant_black(p_bitboard
			//				,alpha,beta,depth,flag_pv_f);
	...
	// насчитываем список избыточных ходов
	// множество этих списков и создают дерево перебора
	list_surplus_moves.public_generator_captures_black(p_bitboard);
	list_surplus_moves.public_generator_move_black(p_bitboard);
	list_surplus_moves.sorting_moves();
...
#if HASH
					// кэшируем лучший на данный момент ход
					HASHM::public_save_best_hash_score_or_move(i, p_bitboard->get_hash_key(), &list_surplus_moves, 
						value, HASHM::LOWER, depth, depth_max);	
#endif//#if HASH
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
	...
	if ( flag_save_best == 1 ) 
	{
		...
		HASHM::public_save_best_hash_score_or_move(i_hash_moves_1, p_bitboard->get_hash_key(), &list_surplus_moves, 
			beta, HASHM::EXACT, depth, depth_max);
	}
	else
	{
		// кэшируем ход дающий локальный минимум т.е. флаг 3
		HASHM::public_save_best_hash_score_or_move(i_hash_moves_3, p_bitboard->get_hash_key(), &list_surplus_moves, 
...
#if !FORCED 
	score = evaluate.public_eval_state(p_bitboard) + (p_bitboard->get_value_white_material() 
