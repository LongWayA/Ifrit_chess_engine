 ...
 * ПОСЛЕДНЕЕ ИЗМЕНЕНИЕ 22.02.2009   11.11.2009 21.11.2010
*/
//-----------------------------------------------------------------------------------
...
void Chess_bitboard::public_parse_position
(
...
)
{
	...
	ZOBRIST::public_start_position_random_key(this);

	// записываем хеш-ключ в массив хеш - ключей по строке игры
	ZOBRIST::public_save_hash_string_moves(hash_key,cnt_move); 
	//---------------------------------------------
...
)
{
	...
	*/
	...
	if (fen[i] != '-')
	{
		...
		taking_passage_x = koordinata_to_indeks(fen[i]);// х координата(из буквы)
		i = i + 1;
		taking_passage_y = ch_koordinata_to_indeks(fen[i]);// у координата 	
	}
	//---------------------------------------------------------------------------
...
)
{
	...
	if (colour_move == 1)
	{
		list_surplus_moves_f.public_generator_captures_white(this);
		list_surplus_moves_f.public_generator_move_white(this);
	}
	else
	{
		list_surplus_moves_f.public_generator_captures_black(this);
		list_surplus_moves_f.public_generator_move_black(this);
	}
	...
	else
	{
		...
		if (colour_move == 1)
		{
			public_onward_moves_white((unsigned __int8)n_3
				,undo,&list_surplus_moves_f);
		}
		else
		{
			public_onward_moves_black((unsigned __int8)n_3
				,undo,&list_surplus_moves_f);
		}
