...
public:
	...
	inline unsigned __int64 get_white_piece(unsigned char name_piece)
	{
		return white_piece[name_piece];
	};

	// битовые списки черных фигур
	inline unsigned __int64 get_black_piece(unsigned char name_piece)
	{
	...
	//  имя фигуры от координаты 
	inline unsigned __int8 get_white_name_from_coord(unsigned char coordinate_piece)
	{
		return white_name_from_coord[coordinate_piece];
	};

	// имя фигуры от координаты 
	inline unsigned __int8 get_black_name_from_coord(unsigned char coordinate_piece)
	{
...
// a_Chess_bitboard_make_move.cpp------------------------------------------
	...
	void public_onward_moves_white
	(
		const unsigned __int8 & i,//номер хода по списку
		struct T_undo undo[],//информация для отката хода
		const class List_surplus_moves * const p_list_surplus_moves//список возможных ходов
	);

	//---------------------------------------------------------------------------
	// реализуем ходы вперед для черных
	void public_onward_moves_black
	(
		const unsigned __int8 & i,//номер хода по списку
		struct T_undo undo[],//информация для отката хода
		const class List_surplus_moves * const p_list_surplus_moves//список возможных ходов
