...
)
{
	...
	{
		name = p_bitboard->get_white_name_from_coord(i);

		if (name != 0) key = key ^ random_piese_key[1][name][i];

		name = p_bitboard->get_black_name_from_coord(i);
	...
	if (p_bitboard->get_colour_move()) key = key ^ U64(0x7C0228DD624EC390);

	// учитываем рокировки
	if (p_bitboard->get_castling_K()) key = key ^ U64(0xB3598055CE64A656);
	if (p_bitboard->get_castling_Q()) key = key ^ U64(0x12A8F886AF9418C2);
	if (p_bitboard->get_castling_k()) key = key ^ U64(0x1877B67E57A764D5);
	if (p_bitboard->get_castling_q()) key = key ^ U64(0xD4490AD346F14431);

	// учитываем взятие на проходе
	if (p_bitboard->get_taking_passage_yes())
	{
		key = key ^ U64(0xA3853B80F17F58EE);
		key = key ^ (p_bitboard->get_taking_passage_x() ^ U64(0x5B23E522E4B1B4EF));
		key = key ^ (p_bitboard->get_taking_passage_y() ^ U64(0x934E1DE72D36D310));
	}//

	p_bitboard->set_hash_key(key);
...
)
{
	...
	key = p_bitboard->get_hash_key();
	name_figure = ((p_list_surplus_moves->get_move(i)>>24)& 7);
	initial_position = ((p_list_surplus_moves->get_move(i)>>12)& 63);
	final_position = ((p_list_surplus_moves->get_move(i)>>18)& 63);
	type_move = ((p_list_surplus_moves->get_move(i)>>6)& 63);
	name_figure_capture = (p_list_surplus_moves->get_move(i) & 63);
	...
	if (p_bitboard->get_castling_K() != undo->return_castling_K) key = key ^ U64(0xB3598055CE64A656);//?
	if (p_bitboard->get_castling_Q() != undo->return_castling_Q) key = key ^ U64(0x12A8F886AF9418C2);
	if (p_bitboard->get_castling_k() != undo->return_castling_k) key = key ^ U64(0x1877B67E57A764D5);
	if (p_bitboard->get_castling_q() != undo->return_castling_q) key = key ^ U64(0xD4490AD346F14431);
