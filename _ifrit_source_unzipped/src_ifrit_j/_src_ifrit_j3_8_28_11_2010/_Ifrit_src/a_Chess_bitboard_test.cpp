...
)
{
	...
	all_piece = p_bitboard->all_piece;// все фигуры
	all_white_piece = p_bitboard->all_white_piece;// все белые фигуры
	all_black_piece = p_bitboard->all_black_piece;// все черные фигуры
	colour_move = p_bitboard->colour_move;
	taking_passage_yes = p_bitboard->taking_passage_yes;
	taking_passage_x = p_bitboard->taking_passage_x;
	taking_passage_y = p_bitboard->taking_passage_y;
	castling_Q = p_bitboard->castling_Q;
	castling_K = p_bitboard->castling_K;
	castling_q = p_bitboard->castling_q;
	castling_k = p_bitboard->castling_k;
	hash_key =  p_bitboard->hash_key;
	value_white_material = p_bitboard->value_white_material;
	value_black_material = p_bitboard->value_black_material;

	for (unsigned __int8 n =0; n < 7; n++ )
	{
		white_piece[n] = p_bitboard->white_piece[n];
		black_piece[n] = p_bitboard->black_piece[n];
	}

	for (unsigned __int8 k =0; k < 64; k++ )
	{
		white_name_from_coord[k] = p_bitboard->white_name_from_coord[k];
		black_name_from_coord[k] = p_bitboard->black_name_from_coord[k];
	}
}
...
)
{
	...
	if (hash_key !=  p_bitboard->hash_key)
	{
		std::cout << "white test_hash_key !=  bitboard->hash_key" << "\n";
	}

	if (value_white_material != p_bitboard->value_white_material)
	{
		std::cout << "white test_value_white_material != bitboard->value_white_material" << "\n";
	}

	if (value_black_material != p_bitboard->value_black_material)
	{
	...
	{
		if (white_piece[n2] != p_bitboard->white_piece[n2])
		{
			std::cout << "white test_white_piece[n] = bitboard->white_piece[n]" << n2 << "\n";
		}

		if (black_piece[n2] != p_bitboard->black_piece[n2])
		{
	...
	{
		if (white_name_from_coord[k2] != p_bitboard->white_name_from_coord[k2])
		{
			std::cout << "white test_white_name_from_coord[k] = bitboard->white_name_from_coord[k]" << k2 << "\n";
		}

		if (black_name_from_coord[k2] != p_bitboard->black_name_from_coord[k2])
		{
