...
//TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
	...
	*/
	...
	// сейчас это в ~17 раз
	ROOTSEARCH::public_ini_deep_time_terminate( (max_time_move_search * 30) );

	//-------------------------------------------------------------------------------


	// считаем материал. в дальнейшем только корректируем оценку при взятиях.
	p_bitboard->set_value_white_material(evaluate.public_eval_white_material(p_bitboard));
	p_bitboard->set_value_black_material(evaluate.public_eval_black_material(p_bitboard));
	...
	ZOBRIST::public_save_hash_three(p_bitboard->get_hash_key(),0);
	...
	{  
		...
		root_list_surplus_moves.public_generator_captures_white(p_bitboard);// генерируем взятия и превращения
		root_list_surplus_moves.public_generator_move_white(p_bitboard);// генерируем остальные ходы
	}
	else
	{
		// черные
		//Заполняем список избыточных ходов. Именно эти списки и составляют дерево перебора.
		root_list_surplus_moves.public_generator_captures_black(p_bitboard);// генерируем взятия и превращения
		root_list_surplus_moves.public_generator_move_black(p_bitboard);// генерируем остальные ходы
	}
