...
/*
 ...
 по позиции заданной в структуре Chess_bitboard генерирует список ходов T_list_surplus_moves
...
void List_surplus_moves::public_generator_captures_white
(
...
)
{
		...
		*/
							...
							// пешки с превращением
		{
			...
			while (captures_conversion_mask != 0)
			{
				...
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_QUEEN,p,from,to,p_bitboard->black_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_ROOK,p,from,to,p_bitboard->black_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_BISHOP,p,from,to,p_bitboard->black_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_KNIGHT,p,from,to,p_bitboard->black_name_from_coord[to]);
		...
		else
		{
			...
			while (captures_mask != 0)
			{
				...
				list_capture_white(N_PAWN,CAPTURE_MOVE,from,to,p,p_bitboard);
		...
		if (p_bitboard->taking_passage_yes == 1)
		{
			...
			if ( from / 8 == 4 )
			{
				...
				{
					list_record_special(N_PAWN,TAKING_PASSAGE,p,from,to,p_bitboard->black_name_from_coord[to - 8]);
	...
	while (from_mask != 0)
	{
		...
		{
			to = bit_scan_forward_win32(captures_mask);
			list_capture_white(N_KNIGHT,CAPTURE_MOVE,from,to,p,p_bitboard);
			captures_mask = captures_mask & (captures_mask - 1);
		}
	}
	// коня ----------------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_white(N_BISHOP,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_white(N_ROOK,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_white(N_QUEEN,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	if (from_mask != 0)
	{
		...
		while (captures_mask != 0)
		{
			...
			to = bit_scan_forward_win32(captures_mask);
			list_capture_white(N_KING,CAPTURE_MOVE,from,to,p,p_bitboard);
			captures_mask = captures_mask & (captures_mask - 1);
...
void List_surplus_moves::public_generator_captures_black
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
	...
	while (from_mask != 0)
	{
		...
		if ( from / 8 == 1 )
		{
			...
			while (captures_conversion_mask != 0)
			{
				...
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_QUEEN,p,from,to,p_bitboard->white_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_ROOK,p,from,to,p_bitboard->white_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_BISHOP,p,from,to,p_bitboard->white_name_from_coord[to]);
				list_record_special(N_PAWN,CAPTURE_WITH_TRANSFORMATION_IN_KNIGHT,p,from,to,p_bitboard->white_name_from_coord[to]);
		...
		else
		{
			...
			{
				to = bit_scan_forward_win32(captures_mask);
				list_capture_black(N_PAWN,CAPTURE_MOVE,from,to,p,p_bitboard);
				captures_mask = captures_mask & (captures_mask - 1);
			}
		}
		...
		if (p_bitboard->taking_passage_yes == 1)
		{
			if ( from / 8 == 3 )
			{
				...
				{
					list_record_special(N_PAWN,TAKING_PASSAGE,p,from,to,p_bitboard->white_name_from_coord[to + 8]);
	...
	while (from_mask != 0)
	{
		...
		{
		   to = bit_scan_forward_win32(captures_mask);
		   list_capture_black(N_KNIGHT,CAPTURE_MOVE,from,to,p,p_bitboard);
		   captures_mask = captures_mask & (captures_mask - 1);
		}
	}
	// коня ----------------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_black(N_BISHOP,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_black(N_ROOK,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		{ 
			...
			if (captures_mask != 0)
			{
				list_capture_black(N_QUEEN,CAPTURE_MOVE,from,blocking_coordinate,p,p_bitboard);
			}
		}
		//---------------------------------------------------------------------------
	...
	if (from_mask != 0)
	{
		...
		{
			to = bit_scan_forward_win32(captures_mask);
			list_capture_black(N_KING,CAPTURE_MOVE,from,to,p,p_bitboard);
			captures_mask = captures_mask & (captures_mask - 1);
		}
	}
	// короля---------------------------------------------------------------------------------
...
//(для белых)
void List_surplus_moves::public_generator_move_white
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
	...
	if (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			...
			list_record_move(N_KING,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
		...
		if (p_bitboard->castling_Q)
		{
			...
			if (((~p_bitboard->all_piece) & CASTLING_Q ) == CASTLING_Q)
			{
				...
				if ((white_check_square(C1,p_bitboard) == 0) 
				&& (white_check_square(D1,p_bitboard) == 0) 
				&& (white_check_square(E1,p_bitboard) == 0))
				{
		...
		if (p_bitboard->castling_K)
		{
			...
			if (((~p_bitboard->all_piece) & CASTLING_K ) == CASTLING_K)
			{
				...
				if ((white_check_square(E1,p_bitboard) == 0)
				&& (white_check_square(F1,p_bitboard) == 0)
				&& (white_check_square(G1,p_bitboard) == 0))
				{
	...
	while (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			to = bit_scan_forward_win32(move_mask);
			move_mask = move_mask & (move_mask - 1);
			list_record_move(N_QUEEN,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			to = bit_scan_forward_win32(move_mask);
			move_mask = move_mask & (move_mask - 1);
			list_record_move(N_ROOK,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			to = bit_scan_forward_win32(move_mask);
			move_mask = move_mask & (move_mask - 1);
			list_record_move(N_BISHOP,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
		//---------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			to = bit_scan_forward_win32(move_mask);
			move_mask = move_mask & (move_mask - 1);
			list_record_move(N_KNIGHT,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
	}
	// коня ----------------------------------------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		while (move_mask != 0)
		{
			to = bit_scan_forward_win32(move_mask);
			move_mask = move_mask & (move_mask - 1);
			list_record_move(N_PAWN,SIMPLE_MOVE,from,to,p,p_bitboard);
		}
	}
	// пешки----------------------------------------------------------------------------------
...
void List_surplus_moves::public_generator_move_black
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
	...
	if (from_mask != 0)
	{
		...
		if (p_bitboard->castling_q)
		{
			...
			if ((~(p_bitboard->all_piece) & CASTLING_q )== CASTLING_q)
			{
				...
				if ((black_check_square(C8,p_bitboard) == 0)
				&& (black_check_square(D8,p_bitboard) == 0)
				&& (black_check_square(E8,p_bitboard) == 0))
				{
					list_record_special(N_KING,LONG_CASTLING,p,from,to,0);			 
				}
			}
		...
		if (p_bitboard->castling_k)
		{
			...
			if ((~(p_bitboard->all_piece) & CASTLING_k )== CASTLING_k)
			{
				...
				if ((black_check_square(E8,p_bitboard) == 0)
				&& (black_check_square(F8,p_bitboard) == 0)
				&&(black_check_square(G8,p_bitboard) == 0))
				{
					list_record_special(N_KING,SHORT_CASTLING,p,from,to,0);
				}
			}
...
unsigned __int8 List_surplus_moves::public_king_white_check
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
...
//	===================================================================
// под боем ли король если шаха нет то функция возвращает 0 иначе цифру шахующей фигуры
unsigned __int8 List_surplus_moves::public_king_black_check
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
...
)
{
	...
	//std::cout << "p_bitboard->black_name_from_coord[to]= "<< ( move[p] & 63) << "\n";
	//
...
)
{
	...
	//std::cout << "p_bitboard->white_name_from_coord[to]= "<< ( move[p] & 63) << "\n";
	//
	//std::cout << "type_move= "<< type_move << "\n";
	//std::cout << "type_move= "<< ((move[count]>>6)& 63)  << "\n";
	//
...
#if TEST_B
void List_surplus_moves::Bitboard_print
(
	const class Chess_bitboard * const p_bitboard// представление доски
)
{
	...
	Test_Bitboard << "bitboard->taking_passage_x= " << (int)p_bitboard->taking_passage_x << "\n";
	Test_Bitboard << "p_bitboard->taking_passage_y= " << (int)p_bitboard->taking_passage_y << "\n";
