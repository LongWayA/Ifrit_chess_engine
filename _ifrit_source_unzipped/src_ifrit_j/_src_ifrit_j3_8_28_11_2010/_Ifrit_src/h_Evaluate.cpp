	...
	*/
	...
	if (from_mask != 0)
	{
		...
		if (stadia_b > rubikon)
		{
			...
			if (((p_bitboard->white_piece[N_ROOK] & 192) != 0) //  ладья заперта на 6 или 7 клетках
			...
			if (((p_bitboard->white_piece[N_ROOK] & 3) != 0) //  ладья заперта на 0 или 1 клетках
	...
	if (from_mask != 0)
	{
		...
		if (stadia_w > rubikon)
		{
			...
			if (( (p_bitboard->black_piece[N_ROOK] 
...
)
{
	...
	*/
	...
	while (from_mask != 0)
	{
		...
		if (!(e_rook_up_masks[from] & p_bitboard->black_piece[N_PAWN]))
		{ 
...
// черные-------------------------------------------------
	...
	while (from_mask != 0)
	{
		...
		// определяем проходные
		// в случае если перед пешкой нет вражеской пешки
		if (!(e_rook_down_masks[from] & p_bitboard->white_piece[N_PAWN]))
		{ 
			eval = eval - PERSPECTIVE_PAWNN;
...
)
{
	...
	if (king_color == 1)
	{
		...
		if (p_bitboard->white_name_from_coord[king_from + 7] != N_PAWN)
		{
			if (king_from != 0)
			{
		...
		if (p_bitboard->white_name_from_coord[king_from + 8] != N_PAWN) 
		{
			value_king_shield = value_king_shield + SHIELD;
		}

		// смотрим на клетку вправо вверх
		if (p_bitboard->white_name_from_coord[king_from + 9] != N_PAWN)
		{
			if (king_from != 7)
			{
				value_king_shield = value_king_shield + SHIELD;
	...
	else
	{
		...
		if (p_bitboard->black_name_from_coord[king_from - 7] != N_PAWN)
		{
			if (king_from != 63)
			{
				value_king_shield = value_king_shield + SHIELD;
		...
		if (p_bitboard->black_name_from_coord[king_from - 8] != N_PAWN)
		{
			value_king_shield = value_king_shield + SHIELD;
		}

		// смотрим на клетку вправо вниз
		if (p_bitboard->black_name_from_coord[king_from - 9] != N_PAWN)
		{
			if (king_from != 56)
			{
				value_king_shield = value_king_shield + SHIELD;
