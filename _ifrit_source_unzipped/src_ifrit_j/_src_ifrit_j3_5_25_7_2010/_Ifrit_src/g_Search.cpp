 ...
 * ПОСЛЕДНЕЕ ИЗМЕНЕНИЕ 2.11.2009 20.12.2009 25.07.2010
*/
//---------------------------------------------------------------------------
...
namespace 
{
	...
	// RAZORING
	const __int32 FUTILITY_MARGIN = 130; // бонус для отсечки на глубине (depth_max - depth) == 1
	const __int32 RAZOR_MARGIN = 300; // бонус для отсечки на глубине (depth_max - depth) <= 3
...
#if (SAVE_LINE && ALPHA_BETA_ON && RAZORING)
...
---------------------------
тест на тактику
1 if (value > beta)     value - FUTILITY_MARGIN; 14 20 одну решил даже лучше чем задуманно

2 if (value < beta)      value + FUTILITY_MARGIN; 15 из 20

*/

	if (!flag_pv_f)
	{ 
		if (((depth_max - depth) == 1)//futility
		...
		{
			// обычно заданно в виде value < beta - razor_margin
			// это то же самое что  value + razor_margin < beta
			value = p_bitboard->value_white_material - p_bitboard->value_black_material
				- FUTILITY_MARGIN;

			if (value > beta)
			{
				__int32 quiescence_value = 
					QUIESCENCESEARCH::public_forced_variant_white(p_bitboard
						,alpha,beta,depth,flag_pv_f);

				if (quiescence_value > beta)  
				{
					return quiescence_value; 
				}
			}
		}
		else if (((depth_max - depth) <= 3)//razoring
		&& (description_move != MOVE_IS_CAPTURE) // если ход не взятие
		&& (flag_null_m == NULL_NO) // если мы не в ветке нулевого хода
		&& (old_flag_check == CHECK_NO)) // если шаха нет
		{
...
#endif//#if (SAVE_LINE && ALPHA_BETA_ON && RAZORING)
...
)
{
	...
	{ 
		...
		&& (old_flag_check == CHECK_NO)) // если шаха нет
		{
			...
			{
				__int32 quiescence_value = 
					QUIESCENCESEARCH::public_forced_variant_black(p_bitboard
						,alpha,beta,depth,flag_pv_f);

				if (quiescence_value < alpha)
				{
					return quiescence_value; 
				}
			}
...
#endif// (SAVE_LINE && ALPHA_BETA_ON && RAZORING) 
