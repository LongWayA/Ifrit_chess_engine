...
public:
	...
	inline unsigned __int64 get_white_piece(const unsigned __int8 name_piece)
	{
		return white_piece[name_piece];
	};

	// битовые списки черных фигур
	inline unsigned __int64 get_black_piece(const unsigned __int8 name_piece)
	{
	...
	//  имя фигуры от координаты 
	inline unsigned __int8 get_white_name_from_coord(const unsigned __int8 coordinate_piece)
	{
		return white_name_from_coord[coordinate_piece];
	};

	// имя фигуры от координаты 
	inline unsigned __int8 get_black_name_from_coord(const unsigned __int8 coordinate_piece)
	{
...
public:
	...
	// 64 битный хеш-ключ позиции
	inline void set_hash_key(const unsigned __int64 _hash_key)
	{
		hash_key = _hash_key;
	};

	// цвет хода 0 - черные 1 - белые
	inline void set_colour_move(const bool _colour_move)
	{
		colour_move = _colour_move;
	};

	// разрешение взятия на проходе 1/0
	inline void set_taking_passage_yes(const bool _taking_passage_yes)
	{
	...
	// х координата битого поля(конвертируем из буквы)
	// так а - 0 , h - 7 таким образом по х отображение прямое
	inline void set_taking_passage_x(const unsigned __int8 _taking_passage_x)
	{
	...
	// у координата битого поля
	// так '1' - 0 , '8' - 7 таким образом отображение тоже прямое только сдвинуто на 1
	inline void set_taking_passage_y(const unsigned __int8 _taking_passage_y)
	{
		taking_passage_y = _taking_passage_y;
	};

	// оценка позиции материал за белых
	inline void set_value_white_material(const unsigned __int16 _value_white_material)
	{
		value_white_material = _value_white_material;
	};

	// оценка позиции материал за черных
	inline void set_value_black_material(const unsigned __int16 _value_black_material)
	{
...
// реализация в файле
// Chess_bitboard_delta_pruning.cpp
//////////////////////////////////////////////////////////////////////////////////////////
public:
...
public:
	...
	// и именно поэтому она в этом, а не в другом классе 
	// печатаем битбоард(bit board) доску
	void bitboard_print();

};

#endif // !defined CHESS_BITBOARD_H