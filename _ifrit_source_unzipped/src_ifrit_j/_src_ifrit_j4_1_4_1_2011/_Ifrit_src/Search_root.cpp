...
#if (TEST_L && SAVE_LINE)
	search.list_print(depth_max,list_surplus_moves);
...
//-----------------------------------------------
		...
		search.extension_for_chek_white(bitboard,depth_max,depth_max_new,flag_check,list_surplus_moves);
...
//AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
		...
		if (search.get_stop_search() != 1)
		{
			...
			// для теста вариантов
			pv_save.print_pv_line(1,list_surplus_moves,pv_save.get_pv_best(),nodes_root,i,depth_max,value,1);
#else
			// для рабочего режима
...
#if (EXTENSION_CHEK && SAVE_LINE)
		// если король под шахом то увеличиваем максимальную глубину перебора 
		search.extension_for_chek_black(bitboard,depth_max,depth_max_new,flag_check,list_surplus_moves);
#endif//#if EXTENSION_CHEK
//-----------------------------------------------
...
)
{
	...
	{	
		...
		Test_Loop << "Ходящая фигура " << bitboard.public_int_to_char_board(((list_surplus_moves.move[i]>>24)& 7),1)<< "\n";
		...
		Test_Loop << "Взятая фигура " << util_o.public_int_to_char_board(( list_surplus_moves.move[i] & 63),1)<< "\n";
