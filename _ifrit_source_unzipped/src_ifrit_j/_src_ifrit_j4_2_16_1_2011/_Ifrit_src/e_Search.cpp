...
//#endif//#if LMR
	...
	for (unsigned __int8 i = 0; i < list_surplus_moves.get_end_list(); i++)
	{
		...
		zobrist.modification_random_key(1,i,list_surplus_moves,bitboard.get_d_undo(depth));
...
//_scout_scout white
...
#endif//#if ALPHA_BETA_ON
		{
			...
			value = black(alpha,beta_scout,depth_max_new_scout,new_depth,description_move,
...
)
{
	...
	score = search_quiescence.forced_variant_white(alpha,beta
		,depth,flag_pv_f);
...
)
{
	...
	score = search_quiescence.forced_variant_black(alpha,beta
		,depth,flag_pv_f);
