...
void ZObrist::start_position_random_key()
{
	...
	if (bitboard.get_colour_move()) key = key ^ 8935749541958697872;

	// учитываем рокировки
	if (bitboard.get_castling_K()) key = key ^ 12923501711739627094;
	if (bitboard.get_castling_Q()) key = key ^ 1344597746125117634;
	if (bitboard.get_castling_k()) key = key ^ 1763078432891561173;
	if (bitboard.get_castling_q()) key = key ^ 15296769511885194289;

	// учитываем взятие на проходе
	if (bitboard.get_taking_passage_yes())
	{
		key = key ^ 11782889425078278382;
		key = key ^ (bitboard.get_taking_passage_x() ^ 6567344619664815343);
		key = key ^ (bitboard.get_taking_passage_y() ^ 10614454250492056336);
	}//
...
//  , 205,    227, 229, 230, 232, 234
	...
	final_position = ((list_surplus_moves.get_move(i)>>18)& 63); //warning Lines end: 191
	...
	if (bitboard.get_castling_K() != undo.return_castling_K) key = key ^ 12923501711739627094;//?
	if (bitboard.get_castling_Q() != undo.return_castling_Q) key = key ^ 1344597746125117634;
	if (bitboard.get_castling_k() != undo.return_castling_k) key = key ^ 1763078432891561173;
	if (bitboard.get_castling_q() != undo.return_castling_q) key = key ^ 15296769511885194289;
