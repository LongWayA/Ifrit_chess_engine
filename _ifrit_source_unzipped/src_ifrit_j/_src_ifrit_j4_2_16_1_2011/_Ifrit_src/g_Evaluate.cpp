...
__int32 Evaluate::eval_state()
{
	...
	// обсчитываем короля
	king_pst(from_white_king, from_black_king, stadia_w, stadia_b, eval);

	// обсчитываем пешки
	pawn_pst(from_white_king, from_black_king, stadia_w, stadia_b, eval);

	// обсчитываем ферзь, ладья, слон, конь
	// бонус расположения 
	PST(from_white_king, from_black_king, at_white_king, at_black_king, eval);
	...
	if (from_mask != 0)
	{
		...
		if (stadia_b > RUBIKON)
		{
			...
			eval = eval 
				...
				- king_shield(from,1,PENALTY_SHIELD)
				;
	...
	if (from_mask != 0)
	{
		...
		if (stadia_w > RUBIKON)
		{
			...
			eval = eval 
				...
				+ king_shield(from,0,PENALTY_SHIELD)
				;
