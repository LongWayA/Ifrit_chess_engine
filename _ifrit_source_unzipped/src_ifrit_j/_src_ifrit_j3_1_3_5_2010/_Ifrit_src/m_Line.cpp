...
namespace {
  ...
  __int32 save_value = -999999999; // запоминаем оценку
  unsigned __int8 save_Depth_Max = 0; // запоминаем глубину
...
// рассчитываемый в момент печати ход
 ...
 std::cout <<UTIL::x_to_notation(((p_list_surplus_moves->move[i]>>12)& 63)%8);
 std::cout <<(1 + ((p_list_surplus_moves->move[i]>>12)& 63)/8);
 std::cout <<UTIL::x_to_notation(((p_list_surplus_moves->move[i]>>18)& 63)%8);
...
// печатаем информацию о глубине просмотра
 ...
 std::cout << "\n";
 std::cout.flush();
}//void FULLSEARCH::Print_std::cout_v
//	*******************************************************************
