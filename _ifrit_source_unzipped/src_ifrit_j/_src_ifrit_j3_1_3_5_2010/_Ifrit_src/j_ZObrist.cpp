...
void ZOBRIST::Start_position_random_key(struct T_Bitboard * p_bitboard// представление доски                                    
							          ){
...
void ZOBRIST::Modification_random_key(bool colour_figure,// цвет ходящей стороны
...
// (используем для поиска повторов позиций для определения ничьей)
// для строки игры в текущем варианте
void ZOBRIST::Save_hash_three(unsigned __int64 key,// хеш-ключ(hash-key) 
...
// (используем для поиска повторов позиций для определения ничьей)
// для строки игры переданной из оболочки
void ZOBRIST::Save_hash_string_moves(unsigned __int64 key,// хеш-ключ
...
// (используем для поиска повторов позиций для определения ничьей)
bool ZOBRIST::Analysis_hash_string_moves(unsigned __int8 depth // глубина
								       ){
