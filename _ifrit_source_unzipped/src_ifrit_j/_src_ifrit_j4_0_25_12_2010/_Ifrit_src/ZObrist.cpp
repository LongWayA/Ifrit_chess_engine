...
)
{
	...
	if (bitboard.get_colour_move()) key = key ^ U64(0x7C0228DD624EC390);

	// учитываем рокировки
	if (bitboard.get_castling_K()) key = key ^ U64(0xB3598055CE64A656);
	if (bitboard.get_castling_Q()) key = key ^ U64(0x12A8F886AF9418C2);
	if (bitboard.get_castling_k()) key = key ^ U64(0x1877B67E57A764D5);
	if (bitboard.get_castling_q()) key = key ^ U64(0xD4490AD346F14431);

	// учитываем взятие на проходе
	if (bitboard.get_taking_passage_yes())
	{
		key = key ^ U64(0xA3853B80F17F58EE);
		key = key ^ (bitboard.get_taking_passage_x() ^ U64(0x5B23E522E4B1B4EF));
		key = key ^ (bitboard.get_taking_passage_y() ^ U64(0x934E1DE72D36D310));
	}//
...
)
{
	...
	if (bitboard.get_castling_K() != undo->return_castling_K) key = key ^ U64(0xB3598055CE64A656);//?
	if (bitboard.get_castling_Q() != undo->return_castling_Q) key = key ^ U64(0x12A8F886AF9418C2);
	if (bitboard.get_castling_k() != undo->return_castling_k) key = key ^ U64(0x1877B67E57A764D5);
	if (bitboard.get_castling_q() != undo->return_castling_q) key = key ^ U64(0xD4490AD346F14431);
