...
*/
	...
	{ 
		...
		&& (old_flag_check == CHECK_NO)) // если шаха нет
		{
			...
			if (value >= beta)
			{
				value = bitboard.get_value_white_material() - bitboard.get_value_black_material()
...
//----------------------------
		...
		extension_for_chek_white(bitboard,depth_max,depth_max_new,flag_check);
...
//----------------------------
		...
		extension_for_chek_black(bitboard,depth_max,depth_max_new,flag_check);
#endif//#if EXTENSION_CHEK
...
)
{
	...
	{	
		...
		Test_Loop << "Ходящая фигура " << util_o.public_int_to_char_board(((list_surplus_moves.move[i]>>24)& 7),1)<< "\n";
