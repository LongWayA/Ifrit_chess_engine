...
)
{
	...
	pv_current.move[depth] = p_list_surplus_moves->get_move(i);
}
...
)
{
	...
	std::cout <<util.x_to_notation(((p_list_surplus_moves->get_move(i)>>12)& 63)%8);
	std::cout <<(1 + ((p_list_surplus_moves->get_move(i)>>12)& 63)/8);
	std::cout <<util.x_to_notation(((p_list_surplus_moves->get_move(i)>>18)& 63)%8);
	std::cout <<(1 + ((p_list_surplus_moves->get_move(i)>>18)& 63)/8);
	...
	*/
		...
		)
	{
		...
		else
		{
			...
			for (unsigned __int8 n = 0; n < p_PV_best_point->depth_max; n++)
			{
				std::cout << util.x_to_notation(((p_PV_best_point->move[n]>>12)& 63)%8);
				std::cout << (1 + ((p_PV_best_point->move[n]>>12)& 63)/8);
				std::cout << util.x_to_notation(((p_PV_best_point->move[n]>>18)& 63)%8);
