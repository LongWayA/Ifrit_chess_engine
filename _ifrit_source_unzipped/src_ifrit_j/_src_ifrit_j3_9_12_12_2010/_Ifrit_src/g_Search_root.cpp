...
)
{
	...
	struct Undo undo[1];//информация используемая при возврате хода
...
//-----------------------------------------------
		...
		search.extension_for_chek_white(p_bitboard,depth_max,depth_max_new,flag_check);
...
//-----------------------------------------------
...
#endif//#if ALPHA_BETA_ON
		{
			...
			value = search.black(p_bitboard,alpha,beta,depth_max_new,
		...
		else
		{
			...
			value = search.black(p_bitboard,alpha,(alpha + 1),depth_max_new,
				new_depth,((list_surplus_moves.get_move(i)>>6) & 63),
...
#if (HASH && SAVE_LINE)
	// кэшируем лучший ход
	hash.save_best_hash_score_or_move(i_hash_move, p_bitboard->get_hash_key(), &list_surplus_moves, 
...
#if (EXTENSION_CHEK && SAVE_LINE)
		// если король под шахом то увеличиваем максимальную глубину перебора 
		search.extension_for_chek_black(p_bitboard,depth_max,depth_max_new,flag_check);
#endif//#if EXTENSION_CHEK
//-----------------------------------------------
		...
		{
			value = search.white(p_bitboard,alpha,beta,depth_max_new,
				new_depth,((list_surplus_moves.get_move(i)>>6) & 63),
				PV_YES,flag_check,NULL_NO);
		...
		{
			//рекурсивный вызов функции
			value = search.white(p_bitboard,(beta - 1),beta,depth_max_new,
				new_depth,((list_surplus_moves.get_move(i)>>6) & 63),
				PV_NO,flag_check,NULL_NO);
