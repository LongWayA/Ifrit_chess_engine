...
//TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
	...
	*/
    ...
    //----------------------------------------------------------------------------------
		...
		if (p_bitboard->get_colour_move() == 1)// 1-ход белых 0- ход черных
		{
	...
	// сейчас это в ~17 раз
	search_root.ini_deep_time_terminate( (max_time_move_search * 30) );

	//-------------------------------------------------------------------------------


	// считаем материал. в дальнейшем только корректируем оценку при взятиях.
	p_bitboard->set_value_white_material(evaluate.eval_white_material(p_bitboard));
	p_bitboard->set_value_black_material(evaluate.eval_black_material(p_bitboard));

	// для проверки что после возвращения оценка вернулась к первоначальному положению
    test_eval =  p_bitboard->get_value_white_material() 
	...
	key_control = p_bitboard->get_hash_key();
	...
	zobrist.save_hash_three(p_bitboard->get_hash_key(),0);
	...
	if (p_bitboard->get_colour_move() == 1)
	{  
		// белые 
		//Заполняем список избыточных ходов. Именно эти списки и составляют дерево перебора.
		root_list_surplus_moves.generator_captures_white(p_bitboard);// генерируем взятия и превращения
		root_list_surplus_moves.generator_move_white(p_bitboard);// генерируем остальные ходы
	}
	else
	{
		// черные
		//Заполняем список избыточных ходов. Именно эти списки и составляют дерево перебора.
		root_list_surplus_moves.generator_captures_black(p_bitboard);// генерируем взятия и превращения
		root_list_surplus_moves.generator_move_black(p_bitboard);// генерируем остальные ходы
	}
...
//---------------------------------------------------------------
	...
	)
	{
		...
		if (p_bitboard->get_colour_move() == 1)
		{
			value_root = search_root.white_root(p_bitboard,
				root_list_surplus_moves,Alpha,Beta,depth);
		}
		else
		{
			value_root = search_root.black_root(p_bitboard,
				root_list_surplus_moves,Alpha,Beta,depth);
		}
		...
		if (test_eval != (p_bitboard->get_value_white_material() - p_bitboard->get_value_black_material()))
		{
		...
		if (key_control != p_bitboard->get_hash_key())
		{
			std::cout <<"\n";
		...
		p_bitboard->get_colour_move() == 0 ? inv = -1 : inv = 1;
