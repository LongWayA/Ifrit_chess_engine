...
// global
class GO go_o;// запускаем расчет позиции
class FEN fen_o;// работаем с фен - строкой

extern class HASHM hash_o;// работаем с хеш-таблицей
extern class EVAL eval_o;// статическая оценка позиции(здесь мы ее инициализируем)

// имя и дата выпуска версии
#define NAME_OF_PROGRAM_AND_DATA_OF_RELEASE "id name Ifrit_j2_9_Beta_15_2_2010"


//	===================================================================
// разбираем входящие UCI команды от графической шахматной оболочки
void PROTOKOLUCI::Parse_Protokol_UCI(struct T_Bitboard * p_bitboard,//доска 
...
//--------------------------------------------------------------------------
    ...
    hash_o.Hash_size_ini_default(1);
...
*/
 ...
 //  cout << "Read_hash_size_Mb(string_in) = " << Read_hash_size_Mb(string_in) << "\n";
   ...
   if(Read_hash_size_Mb(string_in) != 0) {
        ...
        hash_o.Hash_size_resize(hash_size_Mb);
...
// >1:isready
    ...
    // здесь мы инициализируем таблицы статической оценки
    eval_o.Ini_parameters_eval();
...
//  разбираем входящую позицию инициализируем внутреннюю доску движка   
    fen_o.parse_position(p_bitboard,string_in); 
...
//  запускаем движок на расчет позиции
    go_o.parse_go(p_bitboard,string_in);
...
}//void PROTOKOLUCI::Parse_Protokol_UCI
//	*******************************************************************

//	===================================================================
// считывает в подстроке вида "Hash value 32" размер хеш-таблицы (в данном случае 32)
unsigned __int16 PROTOKOLUCI::Read_hash_size_Mb(const char string_in[]) {
...
return 0;
}//int PROTOKOLUCI::Read_hash_size
//	*******************************************************************

//	===================================================================
// есть ли в строке искомая подстрока
bool PROTOKOLUCI::string_equal(const char s1[], const char s2[]) {

//функция показывает равны ли строки видимо уровень соответствия?
   return strcmp(s1,s2) == 0;
}//bool PROTOKOLUCI::string_equal
//	*******************************************************************

//	===================================================================
// есть ли в начале строки искомая подстрока
bool PROTOKOLUCI::string_start_with(const char s1[], const char s2[]) {
...
}//bool PROTOKOLUCI::string_start_with
//	*******************************************************************
