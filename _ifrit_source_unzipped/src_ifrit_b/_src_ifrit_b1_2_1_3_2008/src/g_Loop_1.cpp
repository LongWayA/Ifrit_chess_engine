...
int value_best=9999999;   // лучшая в узле  оценка позиции  
 ...
 int hod_castling=0; // легальна ли рокировка
 int hod_check=0;    // под боем ли король(есть ли шах)
 int no_moves=0;     // флаг существования легальных ходов
 int nodes_0=1;      // количество ходов на нулевом уровне
 int number_taking=0;// конечная позиция по взятиям в списке возможных ходов
int key=0;                // хеш ключ
...
int value_best=9999999;   // оценка позиции лучшая в узле 
 int inv=0;          // инвертирование оценки позиции
 int hod_castling=0; // легальна ли рокировка
int hod_check=0;    // по боем ли король(есть ли шах)
int no_moves=0;     // флаг существования легальных ходов
