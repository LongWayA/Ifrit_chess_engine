...
// T_list_surplus_moves list_surplus_moves;//список избыточных ходов
 char * fen;  // указатель на fen
 char * moves;// указатель на moves
...
//белые фигуры
	   ...
	   // ладьи в списке (сейчас имя ладьи 4)
	   ...
	   // ладьи от координаты
		...
		case 'N'://конь
       bitboard->all_white_piece =bitboard->all_white_piece | move_masks_f[n];
       // кони в списке (сейчас имя коня 2)
       ...
       bitboard->white_name_from_coord[n]=2;
		...
		case 'B'://слон
       bitboard->all_white_piece =bitboard->all_white_piece | move_masks_f[n];
       // слоны в списке (сейчас имя слона 3)
       ...
       bitboard->white_name_from_coord[n]=3;
		...
		case 'Q'://ферзь
       bitboard->all_white_piece =bitboard->all_white_piece | move_masks_f[n];
       // ферзи в списке (сейчас имя ферзя 5)
       ...
       bitboard->white_name_from_coord[n]=5;
		...
		case 'K'://король
       bitboard->all_white_piece =bitboard->all_white_piece | move_masks_f[n];
       // король в списке (сейчас имя короля 6)
...
//		 не забыть при дешифровке что у меня массив по у идет сверху и от нуля
//		 а ведь нормальная нотация снизу и от 1	 
// запретили взятие
		...
		if (fen[i]!='-'){
			...
			i=i+1;
			bitboard->taking_passage_y = Ch_Koordinata_To_Indeks(fen[i]);// у координата(инверсия) 	
...
// откуда фигура ходит
short int n=8*Ch_Koordinata_To_Indeks(move_string[2])+Koordinata_To_Indeks(move_string[1]);
// куда фигура ходит
short int n_2=8*Ch_Koordinata_To_Indeks(move_string[4])+Koordinata_To_Indeks(move_string[3]);
// расчитываем все возможные ходы фигуры
Generator_captures(bitboard,list_surplus_moves_f);
Generator_move(bitboard,list_surplus_moves_f);
//
short int n_3=0; // номер хода по списку
...
// если ход в списке совпадает с ходом из мувика тогда..
	if ((n== list_surplus_moves_f->initial_position[i])&&(n_2== list_surplus_moves_f->final_position[i])){
 //    n_3 =i;
//------------------
...
// делаем ход
Onward_moves(n_3,list_surplus_moves_f,bitboard);
...
//	по букве выдает идекс клетки	
	short int Koordinata_To_Indeks(char ch){
			   if (ch =='a') return 0;
			   if (ch =='b') return 1;
			   if (ch =='c') return 2;
			   if (ch =='d') return 3;
...
//	по символу цифры выдает цифру	
	short int Ch_Koordinata_To_Indeks(int ch){
			   if (ch =='1') return 0;
			   if (ch =='2') return 1;
			   if (ch =='3') return 2;
			   if (ch =='4') return 3;
