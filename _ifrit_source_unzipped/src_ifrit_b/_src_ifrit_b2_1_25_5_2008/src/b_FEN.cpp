...
//moves g1f3 d7d5 b2b3 c8g4 c1b2 b8c6 h2h3 
   if (moves != NULL) { // если команда "moves" присутствует
      ...
      while (*ptr != '\0') { 
         ...
         board_from_move(bitboard,move_string);
         while (*ptr == ' ') ptr++;
      }//while (*ptr != '\0') {
    }//if (moves != NULL)
...
void board_from_move(T_Bitboard * bitboard,char move_string[]){
...
// делаем ход
if(bitboard->colour_move == 1){
    Onward_moves_white(n_3,undo,&list_surplus_moves_f,bitboard);
}else{
    Onward_moves_black(n_3,undo,&list_surplus_moves_f,bitboard);
}//
