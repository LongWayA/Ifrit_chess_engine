...
void Save_killer(int i,T_list_surplus_moves * list_surplus_moves,int depth){
 ...
 killer_moves[depth].move = list_surplus_moves->move[i];
...
for (int j=start;(!((((list_surplus_moves->move[j]>>12)& 63)==((killer_moves[depth].move>>12)& 63))&&(((list_surplus_moves->move[j]>>18)& 63)==((killer_moves[depth].move>>18)& 63)))&&(j<list_surplus_moves->end_list));j++);
