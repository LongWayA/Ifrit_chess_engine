...
void transformation_moves(char three_position[],struct T_three_moves * three_moves){
...
// аргументы: 
void parse_go(struct T_Bitboard * bitboard,const char string_in[],char three_position[],int & nodes){

const char * ptr;
int depth = -1;
struct T_three_moves three_moves;//строка ходов в которой ищем повторы 
double wtime= -1.0;
double btime= -1.0; 
double winc= -1.0;
double binc= -1.0;
