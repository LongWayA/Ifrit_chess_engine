...
if (0==list_surplus_moves.end_list){ 
...
value=Forced_variant_black(bitboard,alpha,beta,(depth+1),pv_current,pv_best);
...
// из списка возможных ходов копируем текущий ход в текущий вариант на текущей глубине.
 Update_PV_current(i,depth,pv_current,&list_surplus_moves);
// рекурсивный вызов функции
value=Forced_variant_white(bitboard,alpha,beta,(depth+1),pv_current,pv_best);
// восстанавливаем позицию
...
inline int Sorting_captures_moves(struct T_list_surplus_moves * list_surplus_moves){
 // 
int move_s;
int m=list_surplus_moves->end_captures;
