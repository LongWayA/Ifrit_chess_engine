...
*/
short int  description_move[MAX_MOVES];

// начало списка точнее первая заполненная позиция
short int  start_list[MAX_DEPTH];

// конец списка точнее первая пустая позиция
short int  end_captures[MAX_DEPTH];

// конец списка точнее первая пустая позиция
short int  end_list[MAX_DEPTH];
/*
пример:
0
1 запись 
...
short int  name_taking_figure[MAX_MOVES];
// флаг возврата взятия на проходе 1/0
short int  return_taking_passage[MAX_DEPTH];
// флаг рокировки белых в длинную сторону
short int  return_castling_Q[MAX_DEPTH];
// флаг рокировки белых в короткую сторону
short int  return_castling_K[MAX_DEPTH];
...
struct T_PV {
//// начальное положение фигуры(откуда фигура ходит)
short int  initial_position[70]; 
// конечное положение фигуры(куда фигура ходит)
short int  final_position[70];
// Фигура превращения пешки 0 - нету 2 - ладья 3 - конь 4 - слон 5 - ферзь 
short int  final_pawn[70];
...
// начальное положение фигуры(откуда фигура ходит)
short int  initial_position; 

// конечное положение фигуры(куда фигура ходит)
short int  final_position;
