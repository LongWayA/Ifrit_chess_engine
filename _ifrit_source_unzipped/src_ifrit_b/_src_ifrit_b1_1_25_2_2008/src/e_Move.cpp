...
short int depth =list_surplus_moves->current_depth;
int i_p = 0;
int f_p = 0;
...
i_p=list_surplus_moves->initial_position[i];
f_p=list_surplus_moves->final_position[i];
...
switch (list_surplus_moves->description_move[i]){
   case 1 :
...
// заносим взятую фигуру в список
    list_surplus_moves->name_taking_figure[i] = bitboard->black_name_from_coord[f_p];
...
// если сьедена ладья то запрещаем рокировки с ее участием
 if(list_surplus_moves->name_taking_figure[i] ==4){
	if(f_p == 56){
        list_surplus_moves->return_castling_q[depth]= bitboard->castling_q;
		bitboard->castling_q =0;
	}//if(f_p == 56){
...
//-------------------------------------------------------  
    ...
    list_surplus_moves->name_taking_figure[i] = bitboard->white_name_from_coord[f_p];
...
// заносим взятую фигуру в список
    list_surplus_moves->name_taking_figure[i] = bitboard->black_name_from_coord[(f_p - 8)];
// зануляем взятую фигуру
...
//стираем фигуру на старом месте----------------- 
   ...
   bitboard->all_white_piece = bitboard->all_white_piece ^ move_masks_m[(f_p + 8)];
    ...
    list_surplus_moves->name_taking_figure[i] = bitboard->white_name_from_coord[(f_p + 8)];
...
}//void Onward_moves_816(short int i,T_list_surplus_moves * list_surplus_moves,T_board_list_816 * board_list_816){ 
//	*******************************************************************
...
// востанавливаем имя взятой фигуры
	bitboard->black_name_from_coord[f_p] = list_surplus_moves->name_taking_figure[i];
...
//-------------------------------------------------------  
   ...
   bitboard->black_piece[bitboard->black_name_from_coord[f_p]] = bitboard->black_piece[bitboard->black_name_from_coord[f_p]] ^ move_masks_m[f_p];
    ...
    bitboard->white_name_from_coord[f_p] = list_surplus_moves->name_taking_figure[i];
//рисуем фигуру на новом месте------------------
	bitboard->all_black_piece = bitboard->all_black_piece ^ move_masks_m[i_p];
	bitboard->all_white_piece = bitboard->all_white_piece ^ move_masks_m[f_p];
...
// востанавливаем имя взятой фигуры
	bitboard->black_name_from_coord[(f_p - 8)] = list_surplus_moves->name_taking_figure[i];
...
// битовый список взятой фигуры
   ...
   bitboard->black_piece[bitboard->black_name_from_coord[f_p]] = bitboard->black_piece[bitboard->black_name_from_coord[f_p]] ^ move_masks_m[f_p];
    ...
    bitboard->white_name_from_coord[(f_p + 8)] = list_surplus_moves->name_taking_figure[i];
//рисуем фигуру на новом месте------------------
	bitboard->all_black_piece = bitboard->all_black_piece ^ move_masks_m[i_p];
	bitboard->all_white_piece = bitboard->all_white_piece ^ move_masks_m[(f_p + 8)];
