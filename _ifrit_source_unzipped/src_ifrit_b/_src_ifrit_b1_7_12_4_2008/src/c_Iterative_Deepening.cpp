...
for (int d=1;((d<(depth+1))&&(br==0));d++){
  ...
  if(bitboard->colour_move ==1){
       value_root=White_root(bitboard,list_surplus_moves,Alpha,Beta,d,0,nodes,&PV_current,&PV_best,0,&three_moves,br,0);
  }else{
       value_root=Black_root(bitboard,list_surplus_moves,Alpha,Beta,d,0,nodes,&PV_current,&PV_best,0,&three_moves,br,0);
  }//if(bitboard->colour_move ==1){
