...
// ставим флаг что есть легальные ходы и засчитываем позицию
 ...
 nodes = nodes+1;
	...
	//-----------------------------------------------
       ...
       Extension_for_captures(i,0,list_surplus_moves->description_move[i],depth_max,depth_max_z,depth);
    #endif
	//-----------------------------------------------
...
int Black(int check,struct T_Bitboard * bitboard,int alpha,int beta, int depth_max, int depth,int & nodes,struct T_PV * pv_current,struct T_PV * pv_best, int description_move,struct T_PV * three_moves, int & br, int null_m){
int value;              // текуща€ оценка позиции
