...
value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,0);
list_surplus_moves->current_depth =depth;
...
// рекурсивный вызов функции
value=White(bitboard,list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,0);
list_surplus_moves->current_depth =depth;
...
if ((null_m==0)&&((depth_max-depth) > 3 )){
   null_m=1;// два раза метод в одной ветке не вызываем
   depth_max_z= depth_max - 3;// редукци€ глубины у нас 3
   value=White(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m);
   list_surplus_moves->current_depth =depth;
...
// ставим флаг что есть легальные ходы и засчитываем позицию
 ...
 nodes = nodes+1;
	...
	Extension_for_chek_white(bitboard,depth_max,depth_max_z);
	#endif
	//-----------------------------------------------

	//-----------------------------------------------
	#if EXTENSION_CAPTURES
       Extension_for_captures(list_surplus_moves->description_move[i],list_surplus_moves->description_move[i-250],depth_max,depth_max_z,depth);
    #endif
	//-----------------------------------------------

#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=Black(bitboard,list_surplus_moves,(alpha),(alpha+1),depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,null_m);
	list_surplus_moves->current_depth =depth;
...
if ((null_m==0)&&( (depth_max-depth)>3) ){
   ...
   value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m);
   list_surplus_moves->current_depth =depth;
...
// ставим флаг что есть легальные ходы и засчитываем позицию
 ...
 nodes = nodes+1;
	...
	#if EXTENSION_CHEK
	// если король под шахом то увеличиваем максимальную глубину перебора 
	Extension_for_chek_black(bitboard,depth_max,depth_max_z);
	#endif
	//-----------------------------------------------
...
#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=White(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=White(bitboard,list_surplus_moves,(beta-1),(beta),depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,null_m);
	list_surplus_moves->current_depth =depth;
    if ((value > alpha)&&(value < beta)){
//	if (value < beta){
	// рекурсивный вызов функции
