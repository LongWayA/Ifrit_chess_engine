...
if ((null_m==0)&&((depth_max-depth) > 2 )&&(description_move != 2)){
//if ((null_m==0)&&((depth_max-depth) > 2 )){
   ...
   value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+3,nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m); 
...
//if ((null_m==0)&&((depth_max-depth) > 2 )){
   null_m=1;// два раза метод в одной ветке не вызываем
   depth_max_z= depth_max; //
   value=White(bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+3,nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m);
