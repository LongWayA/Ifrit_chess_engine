...
//if ((null_m==0)&&((depth_max-depth) > 3 )&&(check == 0)){
   ...
   value=Black(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+4,nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m); 
...
//if ((null_m==0)&&((depth_max-depth) > 3 )&&(check == 0)){
   ...
   null_m=1;// два раза метод в одной ветке не вызываем
   depth_max_z= depth_max; //
   value=White(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+4,nodes,pv_current,pv_best,list_surplus_moves->description_move[1],three_moves,br,null_m);
