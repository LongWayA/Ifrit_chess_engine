...
// надо пояснить что конструкция типа b = b & (b-1)  сбрасывает самый правый единичный бит 
    from_mask = from_mask & (from_mask-1);
	ocenka_p= ocenka_p + Pozicion_q[from]+(  63/( ((from_black_king - from)>=0)?(from_black_king - from):-(from_black_king - from) )  );
  }//while(from_mask != 0){
// ферзя----------------------------------------------------------------------------------
...
//    ocenka_p= ocenka_p + Pozicion_q[from];
    ocenka_p= ocenka_p + Pozicion_q[from] + Rook_amoun_white(bitboard);
  }//while(from_mask != 0){
// ладьи----------------------------------------------------------------------------------
...
// Генерируем ходы++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  while(from_mask != 0){
    from = first_one(from_mask);
    from_mask = from_mask & (from_mask-1);
    ocenka_p= ocenka_p + Pozicion_n[from];
 }// while(from_mask != 0){
// коня ----------------------------------------------------------------------------------
...
// надо пояснить что конструкция типа b = b & (b-1)  сбрасывает самый правый единичный бит 
    from_mask = from_mask & (from_mask-1);
    ocenka_p= ocenka_p - Pozicion_q[from]+(  63/( ((from_white_king - from)>=0)?(from_white_king - from):-(from_white_king - from) )  );;
  }//while(from_mask != 0){
// ферзя----------------------------------------------------------------------------------
...
// Генерируем ходы++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  while(from_mask != 0){
    ...
    from_mask = from_mask & (from_mask-1);
//    ocenka_p= ocenka_p - Pozicion_q[from];
    ocenka_p= ocenka_p - Pozicion_q[from] - Rook_amoun_black(bitboard);
  }//while(from_mask != 0){
...
// штрафуем за сдвоенные пешки
  for(int ii=0;ii<8;ii++){
    if(Structure_pawn_w[ii]>1)ocenka_p=ocenka_p - 2*Structure_pawn_w[ii];
    if(Structure_pawn_b[ii]>1)ocenka_p=ocenka_p + 2*Structure_pawn_b[ii];
...
// идем на клетку вверх
       if(bitboard->white_name_from_coord[n + 7]==0) st=st+10;
// идем на клетку влево вверх
       if(bitboard->white_name_from_coord[n + 8]==0) st=st+10;
// идем на клетку вправо вверх
       if(bitboard->white_name_from_coord[n + 9]==0) st=st+10;
	}else{
// идем на клетку вниз
       if(bitboard->black_name_from_coord[n - 7]==0) st=st+10;
// идем на клетку  влево вниз
       if(bitboard->black_name_from_coord[n - 8]==0) st=st+10;
// идем на клетку вправо вниз
       if(bitboard->black_name_from_coord[n - 9]==0) st=st+10;
	}//if(z==1){
