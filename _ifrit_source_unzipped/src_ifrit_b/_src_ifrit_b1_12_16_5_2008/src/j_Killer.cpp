...
for (int j=start;(!((((list_surplus_moves->move[j]>>12)& 63)==killer_moves[depth].initial_position)&&(((list_surplus_moves->move[j]>>18)& 63)==killer_moves[depth].final_position))&&(j<list_surplus_moves->end_list[depth]));j++);	
