...
T_list_surplus_moves list_surplus_moves={0};// список возможных ходов (всевозможные ходы из данного узла)
...
int White_root(int & from,int & to,T_Bitboard * bitboard,int alpha,int beta, int depth_max, int depth,int & nodes,T_PV * pv_current,T_PV * pv_best, int description_move,T_PV * three_moves, int & br, int null_m){
...
value=Black(check,bitboard,&list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves.move[i]>>6)& 63),three_moves,br,0);
list_surplus_moves.current_depth =depth;
...
// выделил из за печати информации в оболочку. поскольку делаем это только на нулевом уровне
int Black_root(int & from,int & to,T_Bitboard * bitboard,int alpha,int beta, int depth_max, int depth,int & nodes,T_PV * pv_current,T_PV * pv_best, int description_move,T_PV * three_moves, int & br, int null_m){
int value=0;              // текуща€ оценка позиции
...
// рекурсивный вызов функции
value=White(check,bitboard,&list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves.move[i]>>6)& 63),three_moves,br,0);
list_surplus_moves.current_depth =depth;
...
//if ((null_m==0)&&((depth_max-depth) > 3 )&&(check == 0)){
   ...
   value=Black(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+4,nodes,pv_current,pv_best,((list_surplus_moves->move[1]>>6)& 63),three_moves,br,null_m); 
...
#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=Black(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves->move[i]>>6)& 63),three_moves,br,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=Black(check,bitboard,list_surplus_moves,(alpha),(alpha+1),depth_max_z,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves->move[i]>>6)& 63),three_moves,br,null_m);
	list_surplus_moves->current_depth =depth;
...
//if ((null_m==0)&&((depth_max-depth) > 3 )&&(check == 0)){
   ...
   null_m=1;// два раза метод в одной ветке не вызываем
   depth_max_z= depth_max; //
   value=White(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,depth+4,nodes,pv_current,pv_best,((list_surplus_moves->move[1]>>6)& 63),three_moves,br,null_m);
...
#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=White(check,bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves->move[i]>>6)& 63),three_moves,br,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=White(check,bitboard,list_surplus_moves,(beta-1),(beta),depth_max_z,(depth+1),nodes,pv_current,pv_best,((list_surplus_moves->move[i]>>6)& 63),three_moves,br,null_m);
	list_surplus_moves->current_depth =depth;
    if ((value > alpha)&&(value < beta)){
//	if (value < beta){
	// рекурсивный вызов функции
...
inline void Extension_for_captures(int i,int start,int description_move,int description_move_up,int depth_max,int & depth_max_new,int depth){
