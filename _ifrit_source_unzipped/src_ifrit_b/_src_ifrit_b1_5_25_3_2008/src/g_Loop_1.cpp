...
value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,0,0);
list_surplus_moves->current_depth =depth;
...
//22222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
	...
	if (br!=1){
	  ...
	  #if TEST_PRINT
	  // дл€ теста вариантов
	  Print_cout(bitboard,list_surplus_moves,*pv_best,nodes_0,nodes,i,depth_max,value,time_nodes);
	  #else
	  // дл€ рабочего режима
	  Print_cout(bitboard,list_surplus_moves,pv_best_point,nodes_0,nodes,i,depth_max,value_max,time_nodes);
	  #endif
    }//if (br!=1){
    //-----------------------------------------------
...
// рекурсивный вызов функции
value=White(bitboard,list_surplus_moves,alpha,beta,depth_max,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,0,0);
list_surplus_moves->current_depth =depth;
...
//22222222222222222222222222222222222222222222222222222222222222222222222222222222222222222222
	...
	if (br!=1){
	  ...
	  #else
	  // дл€ рабочего режима
	  Print_cout(bitboard,list_surplus_moves,pv_best_point,nodes_0,nodes,i,depth_max,value_min,time_nodes);
	  #endif
    }//if (br!=1){
    //-----------------------------------------------
    #endif //#if !COMPILING_1
...
if (depth == depth_max){
  #if !COMPILING_1
    value=Max_depth_black(bitboard,list_surplus_moves,alpha,beta,depth_max,depth,nodes,pv_current,pv_best,three_moves,description_move,br,for_chek);
//  value=0;
  #endif 
 return value ;
}//	if (Depth == Depth_Max){
...
// ставим флаг что есть легальные ходы и засчитываем позицию
 ...
 nodes = nodes+1;
	...
	Extension_for_chek_white(bitboard,for_chek,depth_max,for_chek_l,depth_max_z);
	#endif
	//-----------------------------------------------

#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=Black(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,for_chek_l,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=Black(bitboard,list_surplus_moves,-(alpha-1),-(alpha),depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,for_chek_l,null_m);
...
if (depth == depth_max){
  #if !COMPILING_1
    value=Max_depth_white(bitboard,list_surplus_moves,alpha,beta,depth_max,depth,nodes,pv_current,pv_best,three_moves,description_move,br,for_chek);
//  value=0;
  #endif 
 return value ;
}//	if (Depth == Depth_Max){
...
// ставим флаг что есть легальные ходы и засчитываем позицию
 ...
 nodes = nodes+1;
	...
	#if EXTENSION_CHEK
	// если король под шахом то увеличиваем максимальную глубину перебора 
	Extension_for_chek_black(bitboard,for_chek,depth_max,for_chek_l,depth_max_z);
	#endif
	//-----------------------------------------------

#if !NEGA_SCOUT
// рекурсивный вызов функции
 value=White(bitboard,list_surplus_moves,alpha,beta,depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,for_chek_l,null_m);
 list_surplus_moves->current_depth =depth;
#endif
	...
	// рекурсивный вызов функции
	value=White(bitboard,list_surplus_moves,-(alpha-1),-(alpha),depth_max_z,(depth+1),nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,for_chek_l,null_m);
	// value=-1*Alpha_beta_816(list_surplus_moves,-(alpha),-(alpha+1),depth_max_z,(depth+1),board_list_816,nodes,pv_current,pv_best,list_surplus_moves->description_move[i],three_moves,br,for_chek_l,null_m);
	list_surplus_moves->current_depth =depth;
	if ((value < alpha)&&(value > beta)){
	// рекурсивный вызов функции
...
int Max_depth_white(T_Bitboard * bitboard,T_list_surplus_moves * list_surplus_moves,int alpha,int beta,int depth_max,int depth,int & nodes,T_PV * pv_current,T_PV * pv_best,T_PV * three_moves,int description_move,int & br,int for_chek){
//int score_m=0;
int score=0;
int score_f=0;
int inv=0;          // инвертирование оценки позиции
...
//	===================================================================
// производим расчеты на максимальной глубине
int Max_depth_black(T_Bitboard * bitboard,T_list_surplus_moves * list_surplus_moves,int alpha,int beta,int depth_max,int depth,int & nodes,T_PV * pv_current,T_PV * pv_best,T_PV * three_moves,int description_move,int & br,int for_chek){
//int score_m=0;
int score=0;
int score_f=0;
int inv=0;          // инвертирование оценки позиции
